#!/usr/bin/env node
/**
 * Export a WordPress-plugin-ready copy of BaseLayer Events.
 *
 * Output: /plugins/baselayer-events/ (gitignored) — copy into wp-content/plugins or SVN.
 *
 * Usage: node scripts/create-plugin/baselayer-events.mjs
 */
...
const pkg = path.join(root, 'themes/baselayer/packages/baselayer-events');
const out = path.join(root, 'plugins/baselayer-events');
...
async function main() {
  ...
  await copyFile(
    path.join(pkg, 'baselayer-events.php'),
    path.join(out, 'baselayer-events.php')
  );
  ...
  if (await exists(path.join(pkg, 'config'))) {
    await copyDir(path.join(pkg, 'config'), path.join(out, 'config'));
  }
  if (await exists(path.join(pkg, 'templates'))) {
    await copyDir(path.join(pkg, 'templates'), path.join(out, 'templates'));
  }
