#!/usr/bin/env node
/**
 * Export a WordPress-plugin-ready copy of BaseLayer Editorial.
 *
 * Output: /plugins/baselayer-editorial/ (gitignored)
 *
 * Usage: node scripts/create-plugin/baselayer-editorial.mjs
 */
...
const pkg = path.join(root, 'themes/baselayer/packages/baselayer-editorial');
const out = path.join(root, 'plugins/baselayer-editorial');
...
async function main() {
  ...
  await copyFile(
    path.join(pkg, 'baselayer-editorial.php'),
    path.join(out, 'baselayer-editorial.php')
  );
