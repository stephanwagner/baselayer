 ...
 */
function fs_theme_menu_editor_term_id(int $menu_term_id = 0): int
{
	if ($menu_term_id > 0) {
		return $menu_term_id;
	}
 ...
 * @return string[]
 */
function fs_theme_menu_locations_for_menu(int $menu_term_id): array
{
	if ($menu_term_id <= 0) {
		return [];
	}
 ...
 */
function fs_theme_menu_options_for_menu_term(int $menu_term_id): array
{
	$menu_term_id = fs_theme_menu_editor_term_id($menu_term_id);
	if ($menu_term_id <= 0) {
		return [];
	}

	$by_id = [];
	foreach (fs_theme_menu_locations_for_menu($menu_term_id) as $location) {
		foreach (fs_theme_menu_options($location) as $option) {
	...
	foreach (fs_theme_menus_config() as $menu_config) {
 ...
 */
function fs_menu_item_option_enabled(int $menu_item_id, array $option): bool
{
	if ($menu_item_id <= 0 || $option['id'] === '') {
		return false;
	}

	$meta_key = fs_menu_item_option_meta_key($option['id']);
 ...
 */
function fs_menu_item_render_option_fields($item_id, object $item, int $depth, ?stdClass $args, int $menu_term_id): void
{
	...
	$options = fs_theme_menu_options_for_menu_term($menu_term_id);
	if ($options === []) {
		return;
	}
	...
	foreach ($options as $option) {
		...
		$checked = fs_menu_item_option_enabled($item_id, $option);
		?>
		<p class="field-fs-menu-option field-fs-menu-option-<?= esc_attr($option['id']) ?> description description-wide">
			<label for="<?= esc_attr($field_id) ?>">
				...
				/>
				<?= esc_html__($option['label'], 'fromscratch') ?>
			</label>
		</p>
		<?php
	}
...
add_action('wp_nav_menu_item_custom_fields', 'fs_menu_item_render_option_fields', 10, 5);

/**
 * Save menu item option checkboxes.
 */
function fs_menu_item_save_option_fields(int $menu_id, int $menu_item_id): void
{
	if (!current_user_can('edit_theme_options')) {
		return;
	}

	$options = fs_theme_menu_options_for_menu_term($menu_id);
	foreach ($options as $option) {
		$field_name = 'menu-item-fs-option-' . $option['id'];
		$meta_key = fs_menu_item_option_meta_key($option['id']);
...
add_action('wp_update_nav_menu_item', 'fs_menu_item_save_option_fields', 10, 2);
 ...
 * @return string[]
 */
function fs_menu_item_option_css_classes(array $classes, object $item, object $args, int $depth): array
{
	unset($depth);
	...
	foreach (fs_theme_menu_options($theme_location) as $option) {
		if ($option['className'] === '') {
			continue;
		}
		if (!fs_menu_item_option_enabled((int) $item->ID, $option)) {
			continue;
		}
...
add_filter('nav_menu_css_class', 'fs_menu_item_option_css_classes', 10, 4);
