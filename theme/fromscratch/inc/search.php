 ...
 */
function fs_search_form(array $args = []): string
{
	$defaults = [
		'placeholder' => __('Enter search term…', 'fromscratch'),
		'label' => __('Search for:', 'fromscratch'),
		'button' => __('Search', 'fromscratch'),
		'value' => get_search_query(),
		'action' => fs_search_url(),
		'id' => 'fs-search',
		'class' => '',
		'autofocus' => false,
	];
	...
	ob_start();
	fs_render_template('search-form', $args);
	$html = (string) ob_get_clean();

	/**
	 * @param string $html   Search form markup.
	 * @param array  $args   Arguments passed to {@see fs_search_form()}.
	 */
	return (string) apply_filters('fs_search_form_html', $html, $args);
}

/**
 * Echo {@see fs_search_form()}.
 *
 * @param array<string, mixed> $args
 */
function fs_the_search_form(array $args = []): void
{
	echo fs_search_form($args); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped in template.
}

add_filter('get_search_form', function (string $form, array $args): string {
	return fs_search_form($args);
}, 10, 2);
 ...
 */
function fs_search_modal_markup(): void
{
	?>
...
add_action('wp_footer', 'fs_search_modal_markup', 5);
