 ...
 */
function fs_language_switcher_submenu_arrow_svg(): string
{
 ...
 */
function fs_language_switcher_submenu_item_html(
	string $id,
	string $label,
	string $url,
	bool $is_active,
	...
	$content = fs_language_switcher_item_content($id, $label);
 ...
 */
function fs_language_switcher_dropdown_html(
	...
	$flag_url = fs_language_flag_url($current_lang_id);
	$google_translate = !empty($options['google_translate']);

	$toggle_label = $current_label !== ''
		? sprintf(
			/* translators: %s: current language name */
			__('Select language, current: %s', 'fromscratch'),
			$current_label
		)
		: __('Select language', 'fromscratch');
	...
	<nav
		...
		aria-label="<?= esc_attr__('Languages', 'fromscratch') ?>"
			...
			<li class="menu-item menu-item-has-children menu-depth-0 fs-language-switcher__item">
				...
				>
					<?php if ($flag_url !== '') : ?>
						...
						/>
					<?php endif ?>
					<?= fs_language_switcher_submenu_arrow_svg() ?>
				</button>
