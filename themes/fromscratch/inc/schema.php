 ...
 */
$GLOBALS['fs_schema_faq_items'] = [];

/**
 * Normalize text for FAQ schema (strip tags, collapse whitespace).
 */
function fs_schema_faq_plain_text(string $html): string
{
 ...
 */
function fs_schema_faq_collect(string $question, string $answer_html): void
{
	if (is_admin()) {
		return;
	}

	$question = fs_schema_faq_plain_text($question);
	$answer = fs_schema_faq_plain_text($answer_html);

	if ($question === '' || $answer === '') {
		return;
	}

	if (!isset($GLOBALS['fs_schema_faq_items']) || !is_array($GLOBALS['fs_schema_faq_items'])) {
		$GLOBALS['fs_schema_faq_items'] = [];
	}
 ...
 * @return array<string, mixed>|null
 */
function fs_schema_faq_get_data(): ?array
{
	$items = $GLOBALS['fs_schema_faq_items'] ?? [];
 ...
 */
function fs_schema_faq_footer(): void
{
	if (is_admin()) {
		return;
	}

	$data = fs_schema_faq_get_data();
	if ($data === null) {
		return;
	}
...
add_action('wp_footer', 'fs_schema_faq_footer', 20);