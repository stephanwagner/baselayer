 ...
 */
function fs_install_should_create_child_theme(): bool
{
 ...
 * Reserved parent slug “fromscratch” becomes “fromscratch-child”.
 */
function fs_install_sanitize_theme_slug(string $slug): string
{
	$slug = strtolower(sanitize_title($slug));
	if ($slug === 'fromscratch') {
		return 'fromscratch-child';
	}
	return $slug;
}
 ...
 */
function fs_install_child_theme_templates_dir(): string
{
 ...
 */
function fs_install_create_child_theme(array $theme)
{
	$slug = fs_install_sanitize_theme_slug((string) ($theme['slug'] ?? ''));
	...
	if ($slug === '' || !preg_match('/^[a-z][a-z0-9-]*$/', $slug)) {
		return new WP_Error('fs_child_slug', __('Invalid child theme slug.', 'fromscratch'));
	}

	if ($name === '') {
		return new WP_Error('fs_child_name', __('Theme name is required.', 'fromscratch'));
	}

	$templates_dir = fs_install_child_theme_templates_dir();
	if (!is_dir($templates_dir)) {
		return new WP_Error('fs_child_templates', __('Child theme templates are missing from the parent theme.', 'fromscratch'));
	}
	...
	if (is_dir($child_dir)) {
		return new WP_Error('fs_child_exists', __('A theme or folder with that name already exists. Choose a different theme slug.', 'fromscratch'));
	}
 ...
 * @return true|WP_Error
 */
function fs_install_copy_child_theme_templates(string $source_dir, string $dest_dir, array $tokens)
{
	...
	foreach ($iterator as $item) {
		...
		if ($is_text) {
			...
			if ($contents === false) {
				return new WP_Error('fs_child_read', __('Could not read child theme template files.', 'fromscratch'));
			}
