  ...
  const renderVariantToggle = () => {
    if (ToggleGroupControl && ToggleGroupControlOption) {
      return (
        ...
        >
          <ToggleGroupControlOption value="outline" label={t('outline', 'Outline')} />
          <ToggleGroupControlOption value="fill" label={t('filled', 'Filled')} />
        </ToggleGroupControl>
      );
    }
  ...
  return (
    <div className="fs-icon-picker">
      ...
      <div className="fs-icon-picker__control">
        <Button variant="secondary" className="fs-icon-picker__trigger" onClick={() => setIsOpen(true)}>
            ...
            {iconCategories.map((category) => {
              ...
              return (
                <div key={category.slug} className="fs-icon-picker__category">
                  <h3 className="fs-icon-picker__category-title">{categoryName(category)}</h3>
                    ...
                    {icons.map((icon) => {
                      ...
                      return (
                        <button
                          key={icon.filename}
                          type="button"
                          className={'fs-icon-picker__item' + (value === resolved ? ' is-selected' : '')}
                          onClick={() => {
                            onChange(resolved);
                            setIsOpen(false);
                          }}
                          ...
                          title={name}
                        >
                          <span className={'fs-icon -icon-' + resolved} aria-hidden="true" />
                        </button>
                      );
                    })}
                  </div>
