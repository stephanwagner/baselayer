...
<div class="social-media-links__wrapper">
	<ul class="social-media-links__items">
		<?php foreach ($links as $item) :
			...
			$aria = function_exists('fs_social_media_link_aria_label')
				? fs_social_media_link_aria_label($label)
				: sprintf(
					/* translators: %s: social network name or site host */
					__('Link to %s', 'fromscratch'),
					$label !== '' ? $label : __('Social media', 'fromscratch')
				);
			...
			<li class="social-media-links__item">
				...
				>
					<?php if ($svg !== '') : ?>
						<?= $svg // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- sanitized via fs_svg_sanitize() ?>
					<?php else : ?>
