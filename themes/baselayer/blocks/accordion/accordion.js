 ...
 * Toggle an accordion
 */
$('.bl-wp-block.-baselayer-block .accordion__header').on('click keydown', function (e) {
...
function scrollToAccordionByHash(hash) {
  ...
  const accordionWrapper = $('.bl-wp-block.-baselayer-block.accordion__wrapper[data-accordion-id="' + hashId + '"]').first();
  navigateToAccordion(accordionWrapper);
}

if ($('.bl-wp-block.-baselayer-block.accordion__wrapper[data-accordion-id]').length) {
  $('a[href*="#"]').each(function (index, item) {
    const link = $(item);
    const href = link.attr('href');
    const hrefSplit = href.split('#');
    const accordionWrapper = $('.bl-wp-block.-baselayer-block.accordion__wrapper[data-accordion-id="' + hrefSplit[hrefSplit.length - 1] + '"]').first();
    if (accordionWrapper.length) {
...
function openAccordionWithNeighbours(wrapper, options = {}) {
  if (wrapper.attr('data-close-neighbouring-accordions') === 'true') {
    let wrapperSiblings = $();
    wrapperSiblings = wrapperSiblings.add(wrapper.prevUntil(':not(.bl-wp-block.-baselayer-block.accordion__wrapper)'));
    wrapperSiblings = wrapperSiblings.add(wrapper.nextUntil(':not(.bl-wp-block.-baselayer-block.accordion__wrapper)'));
    wrapperSiblings.filter('.bl-wp-block.-baselayer-block.accordion__wrapper.accordion-open').each(function (index, item) {
      closeAccordion($(item));
    });
  }
