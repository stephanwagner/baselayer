(() => {
  ...
  function createSettingsPanel(initial, definitionType, onChange) {
    ...
    const parseAlignSupports = (raw) => {
      ...
      return String(raw || "").split(/[\s,]+/).map((p) => p.trim().toLowerCase()).filter((p) => p !== "" && allowed.has(p));
    };
    ...
    const syncAlignButtons = () => {
      alignGroup.querySelectorAll("button[data-value]").forEach((node) => {
        const on2 = selectedAlign.includes(node.dataset.value);
    ...
    const alignGroup = el("div", {
      ...
      role: "group",
      "aria-label": t2("settingsAlign", "Alignment supports")
    });
    const appendAlignBtn = (opt) => {
      const label = t2(opt.labelKey, opt.fallback);
      const btn = el("button", {
        type: "button",
        className: "button bl-button-small bl-blocks-align-supports__btn" + (selectedAlign.includes(opt.value) ? " is-active" : ""),
        text: label,
    ...
    if (definitionType === "block") {
      ...
      children.push(
        ...
        el("h3", {
          ...
          text: t2("settingsInnerBlocksSection", "InnerBlocks")
        }),
        innerBlocksRow,
        ...
        el("h3", {
          className: "bl-forms-builder__section-title",
          text: t2("settingsParent", "Parent blocks")
        }),
        parentRow
      );
    }
  ...
  function typeLabel(item) {
    if (item?.kind === "preset") {
      return t3("optionTypePreset", "Preset");
    }
  ...
  function createOptionsPanel(initial, onChange, options = {}) {
    ...
    function renderParamField(paramKey, paramDef, value, onUpdate) {
      ...
      if (ptype === "boolean") {
        const check = el2("input", { type: "checkbox", checked: !!value });
        check.addEventListener("change", () => onUpdate(check.checked));
        return el2("label", { className: "bl-bo-check" }, [
          check,
      ...
      if (ptype === "size" || ptype === "align") {
        ...
        return el2("div", { className: "bl-bo-field" }, [el2("label", { text: label }), select]);
      }
      return el2("div", { className: "bl-bo-field" }, [
        el2("label", { text: label }),
        el2("input", {
          type: "text",
          value: value ?? "",
    ...
    function renderDescriptionField(value, onUpdate) {
      ...
      return el2("div", { className: "bl-bo-field" }, [
        el2("label", { text: t3("optionDescription", "Description") }),
        textarea
      ]);
    }
    ...
    function renderPresetEditor(item) {
      ...
      if (presets.length === 0) {
        slugSelect.appendChild(
          el2("option", {
            value: "",
            text: t3("noPresetsYet", "No presets yet \u2014 create some under Block Options \u2192 Presets")
          })
        );
      } else {
        ...
        if (item.slug && !presets.some((p) => p.slug === item.slug)) {
          slugSelect.appendChild(
            el2("option", { value: item.slug, text: item.slug + " (missing)", selected: true })
          );
        }
      ...
      if (controls.length > 0) {
        ...
        controls.forEach((control) => {
          const section = el2("div", { className: "bl-bo-preset-defaults" });
          const title = control.label || customsCatalog()[control.type]?.label || control.type || control.id;
          section.appendChild(el2("strong", { text: title }));
          ...
          if (isCustomType(control.type)) {
            ...
            section.appendChild(
              el2("div", { className: "bl-bo-field" }, [
                el2("label", { text: t3("defaultValue", "Default") }),
                select
              ])
            );
          }
  ...
  function typeLabel2(type) {
    ...
    if (type === "repeater") {
      return t4("repeaterType", "Repeater");
    }
    return type;
  }
  ...
  function createRepeaterCard(initial = {}, open = false, depth = 1) {
    ...
    const preview = el3("span", { className: "bl-forms-builder__preview" });
    const typeLabelText = () => typeLabel2("repeater") + (depth > 1 ? ` (${depth})` : "");
    const typeChip = el3("span", { className: "bl-forms-builder__field-type" });
    const settingsBtn = el3("button", {
      type: "button",
    ...
    const emptyHint = el3("p", {
      className: "description bl-forms-builder__section-empty",
      text: depth >= REPEATER_MAX_DEPTH ? t4("repeaterEmptyMaxDepth", "Drop fields here (nested repeater not allowed at this depth)") : t4("repeaterEmpty", "Drop fields or a nested repeater here")
    });
    const syncEmpty = () => {
    ...
    const fieldsWrap = el3("div", { className: "bl-forms-builder__repeater-fields-wrap" }, [
      fieldsList,
      emptyHint
    ]);
    syncEmpty();
      ...
      if (logic && logic.enabled && Array.isArray(logic.groups) && logic.groups.length > 0) {
        typeChildren.push(
          el3("span", {
            ...
            title: t4("logicEnable", "Conditional logic"),
            "aria-label": t4("logicEnable", "Conditional logic")
          })
        );
      }
    ...
    const duplicateBtn = el3("button", {
      type: "button",
      className: "bl-forms-builder__icon-btn",
      title: t4("duplicate", "Duplicate"),
      "aria-label": t4("duplicate", "Duplicate"),
      onClick: () => {
    ...
    const deleteBtn = el3("button", {
      type: "button",
      className: "bl-forms-builder__icon-btn bl-forms-builder__icon-btn--danger",
      title: t4("delete", "Delete"),
      "aria-label": t4("delete", "Delete"),
      onClick: () => {
        row.remove();
        notifyChanged();
      }
    ...
    syncCollapse();
    const header = el3("div", { className: "bl-forms-builder__field-header" }, [
      handle,
      preview,
      el3("div", { className: "bl-forms-builder__field-meta" }, [settingsBtn, typeChip]),
      el3("div", { className: "bl-forms-builder__field-actions" }, [
        collapseBtn,
        duplicateBtn,
        deleteBtn
      ])
  ...
  // themes/baselayer/packages/baselayer-blocks/src/js/admin/wysiwyg-card.js
  var TOOLBAR_OPTIONS = [
  ...
  var CODEX_URL = "https://codex.wordpress.org/TinyMCE_Custom_Buttons";
  function createWysiwygHeightSettings(field, FormBuilder) {
    const { el: el8, t: t6 } = FormBuilder;
    if (!el8 || !t6) return [];
    const raw = parseInt(field.height, 10);
    const value = Number.isFinite(raw) && raw >= WYSIWYG_MIN_HEIGHT_PX ? String(raw) : "";
    const input = el8("input", {
      type: "number",
      className: "widefat",
    ...
    return [
      el8("p", {}, [
        el8("label", { text: t6("wysiwygHeight", "Height") }),
        el8("div", { className: "bl-blocks-fields__input-group bl-forms-builder__wysiwyg-height" }, [
          input,
          el8("span", {
      ...
      el8("p", {
        className: "description",
        text: t6("wysiwygHeightHelp", "Editor height in pixels. Leave empty for the default.")
      })
    ];
  }
  function createWysiwygToolbarSettings(field, FormBuilder) {
    const { el: el8, t: t6 } = FormBuilder;
    if (!el8 || !t6) return [];
    const preset = ["basic", "standard", "full", "custom"].includes(field.toolbar) ? field.toolbar : "basic";
    ...
    const select = el8("select", {
      className: "widefat",
      dataset: { blWysiwygToolbar: "1" }
    });
    TOOLBAR_OPTIONS.forEach((opt) => {
      const option2 = el8("option", {
        value: opt.value,
        text: t6(opt.labelKey, opt.fallback),
        selected: preset === opt.value ? true : void 0
      });
      select.appendChild(option2);
    });
    const customWrap = el8("div", {
      className: "bl-forms-builder__wysiwyg-custom",
      hidden: preset !== "custom" ? true : void 0
    });
    const customInput = el8("input", {
      type: "text",
      className: "widefat",
    ...
    customWrap.append(
      el8("p", {}, [
        el8("label", { text: t6("wysiwygCustomButtons", "Custom toolbar buttons") }),
        customInput
      ]),
      el8("p", {
        className: "description",
    ...
    const codeSwitch = el8("div", { className: "bl-forms-builder__switch-setting" }, [
      el8("label", { className: "bl-forms-builder__switch" }, [
        codeInput,
        el8("span", { className: "bl-forms-builder__switch-ui", "aria-hidden": "true" }),
        el8("span", {
          className: "bl-forms-builder__switch-label",
          text: t6("wysiwygAllowCodeEditing", "Allow code editing")
        })
      ]),
      el8("p", {
        className: "description",
    ...
    return [
      el8("div", { className: "bl-forms-builder__wysiwyg-toolbar" }, [
        el8("p", {}, [
          el8("label", { text: t6("wysiwygToolbar", "Toolbar") }),
          select
        ]),
        el8("p", {
          className: "description",
  ...
  function serializeWysiwygToolbar(data, ctx) {
  ...
  var BLOCKS_POPULAR_TYPES = ["text", "textarea", "wysiwyg", "select", "toggle"];
  ...
  function mountApp(root, initial, definitionType = "block") {
    ...
    const syncAll = () => {
      ...
      writeConfig2(payload);
      builderApi?.canvas?.syncEmpty?.();
    };
    const fieldsPanel = el8("div", {
    ...
    root.append(
      el8("div", { className: "bl-forms-builder__scroll" }, [
        el8("div", { className: "bl-forms-builder__scroll-inner" }, [tabBar, panelsWrap])
      ])
    );
  ...
  function whenInDocument(el8, cb) {
    if (!el8 || typeof cb !== "function") return;
    if (el8.isConnected) {
  ...
  function createBooleanControl(field, id, checked) {
    ...
    if (asCheckbox) {
      ...
      if (adjacent) {
        ...
        children2.push(el7("span", { text: adjacent }));
      }
      return el7("label", { className: "bl-blocks-fields__choice" }, children2);
    }
    const children = [
      input,
      el7("span", { className: "bl-blocks-fields__switch-ui", "aria-hidden": "true" })
    ];
    if (adjacent) {
      children.push(el7("span", { className: "bl-blocks-fields__switch-label", text: adjacent }));
    }
    return el7("label", { className: "bl-blocks-fields__switch" }, children);
  }
  ...
  function createLeafControl(field, values, controls) {
    ...
    if (type === "textarea") {
      ...
      options.forEach((opt, i) => {
        ...
        if (asCheckbox) {
          ...
          control.appendChild(
            el7("label", { className: "bl-blocks-fields__switch" }, [
              input,
              el7("span", { className: "bl-blocks-fields__switch-ui", "aria-hidden": "true" }),
              el7("span", { className: "bl-blocks-fields__switch-label", text: optLabel })
            ])
          );
        }
      });
      ...
      const createThumbValue = (className, forId) => el7("output", {
      ...
      const createBoundLabel = (kind) => el7("span", {
        className: `bl-blocks-fields__range-bound is-${kind}`,
        text: formatRangeLabel(kind === "min" ? minFallback : maxFallback)
      });
      const createTrackRow = (stackEl) => el7("div", { className: "bl-blocks-fields__range-track-row" }, [
      ...
      const track = el7("div", { className: "bl-blocks-fields__range-track", "aria-hidden": "true" });
      const fill = el7("div", { className: "bl-blocks-fields__range-fill" });
      track.appendChild(fill);
      const stack = el7("div", { className: "bl-blocks-fields__range-stack" }, [track]);
      if (mode === "single") {
        ...
        const row2 = showInputs ? el7("div", { className: "bl-blocks-fields__range-control-row" }, [numberInput, trackRow]) : trackRow;
        control = el7("div", {
        ...
        const row2 = showInputs ? el7("div", { className: "bl-blocks-fields__range-control-row" }, rowChildren) : trackRow;
        control = el7("div", {
    ...
    if (field.description) {
      row.appendChild(
        el7("p", { className: "description bl-blocks-fields__description", text: field.description })
      );
    }
    return row;
  }
  function createFieldForm(fields, values = {}, options = {}) {
    ...
    const appendColumnGroup = (columns, parent, valueMap) => {
      ...
      const group = el7("div", { className: "bl-blocks-fields__columns" });
