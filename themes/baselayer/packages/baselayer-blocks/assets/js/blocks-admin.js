(() => {
  ...
  function createLeafControl(field, values, controls) {
    ...
    if (type === "textarea") {
      ...
      const minFallback = field.min != null && field.min !== "" ? String(field.min) : "";
      const maxFallback = field.max != null && field.max !== "" ? String(field.max) : "";
      ...
      if (mode === "single") {
        ...
        const fromInput = el7("input", {
          className: "widefat",
          type: "number",
          id,
          value: pair.from,
          "aria-label": i18n6("rangeFrom", "From")
        });
        const toInput = el7("input", {
          className: "widefat",
          type: "number",
          id: id + "-to",
          value: pair.to,
          "aria-label": i18n6("rangeTo", "To")
        });
        applyBounds(fromInput);
        applyBounds(toInput);
        control = el7("div", { className: "bl-blocks-fields__range" }, [
          wrapAffix(fromInput),
          el7("span", { className: "bl-blocks-fields__range-sep", text: "\u2013" }),
          wrapAffix(toInput)
        ]);
