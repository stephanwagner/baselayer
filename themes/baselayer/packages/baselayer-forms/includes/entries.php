...
function bl_forms_entry_columns(array $columns): array
{
	...
	foreach ($columns as $key => $label) {
		if ($key === 'date') {
			$new['bl_form'] = __('Form', 'baselayer-forms');
			$new['bl_mail_admin'] = __('Admin email', 'baselayer-forms');
			$new['bl_mail_user'] = __('User email', 'baselayer-forms');
		}
		$new[$key] = $label;
	}
...
function bl_forms_entry_column_content(string $column, int $post_id): void
{
	...
	if ($column === 'bl_mail_admin' || $column === 'bl_mail_user') {
		...
		if (!empty($mail[$key])) {
			...
			echo '<span style="color:#b32d2e;" title="' . esc_attr((string) $mail[$err]) . '">' . esc_html__('Failed', 'baselayer-forms') . '</span>';
		} else {
