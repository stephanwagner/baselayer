(() => {
  ...
  function assignFiles(input, files, maxFiles = 0) {
    ...
    const limit = multiple ? Math.max(1, Number(maxFiles) || 10) : 1;
  ...
  function collectUploadFiles(form) {
    ...
    form.querySelectorAll("[data-bl-form-file-input]").forEach((input) => {
      const kind = input.closest("[data-bl-form-upload]")?.getAttribute("data-bl-form-upload-kind") || "file";
  ...
  function createProgressController(root) {
    ...
    return {
      ...
      setUpload(files, loaded, total) {
        ...
        const allImages = count > 0 && files.every(
          (item) => item.kind === "image" || isImageFile(item.file, item.kind)
        );
        const template = allImages ? i18n.uploadingImage || "Uploading image %1$s/%2$s" : count === 1 ? i18n.uploadingFile || "Uploading file %1$s/%2$s" : i18n.uploadingFiles || "Uploading %1$s/%2$s";
  ...
  function readLogicSourceValue(form, fieldId) {
    ...
    if (checks.length) {
      if (source.classList.contains("-toggle") || source.classList.contains("-terms")) {
  ...
  function setLogicFieldVisible(wrap, visible) {
    ...
    wrap.querySelectorAll("input, select, textarea, button").forEach((el) => {
      if (visible) {
        ...
        if (nested && nested !== wrap && (nested.hidden || nested.classList.contains("is-bl-logic-hidden"))) {
          return;
        }
  ...
  function initForm(root) {
    ...
    const handleSuccess = (payload) => {
      const redirect = payload && payload.redirect || (payload && payload.entry_id && afterSubmit === "redirect" ? redirectUrl : "") || "";
      if (redirect) {
      ...
      try {
        ...
        const { data } = await postFormData(ajaxUrl, body, {
          onUploadProgress: (event2) => {
            ...
            const total = event2.lengthComputable && event2.total > 0 ? event2.total : uploads.reduce((sum, item) => sum + (item.file.size || 0), 0);
            progress.setUpload(uploads, event2.loaded || 0, total);
          },
          onUploadComplete: () => {
            if (useProgress) {
              progress.setSending();
