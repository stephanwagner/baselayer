 ...
 */
function bl_forms_load_textdomain_mofile(string $mofile, string $domain): string
{
	if ($domain !== BL_FORMS_TEXTDOMAIN) {
		return $mofile;
	}

	$locale = function_exists('determine_locale') ? determine_locale() : get_locale();
	$packaged = bl_forms_packaged_mofile($locale);
	if ($packaged === '') {
		return $mofile;
	}
...
{
	$domain = BL_FORMS_TEXTDOMAIN;
	$mofile = bl_forms_packaged_mofile();
	if ($mofile !== '') {
...
add_filter('load_textdomain_mofile', 'bl_forms_load_textdomain_mofile', 10, 2);
