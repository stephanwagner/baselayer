(function () {
  ...
  const { PanelRow, ToggleControl, Button, Modal, SelectControl, TextControl, TextareaControl, DropdownMenu } =
    wp.components;
  ...
  function EventPanelContent() {
    ...
    useEffect(
      function () {
        ...
        function flushDateDrafts() {
          ...
          if (next[META_END_DATE] !== undefined) {
            setEndDraft(next[META_END_DATE]);
          }
          if (next[META_START_DATE] !== undefined) {
        ...
        function isEditorSaveControl(target) {
          ...
          return !!(
            ...
            target.closest('.editor-post-save-draft') ||
            target.closest('.editor-post-publish-panel__toggle') ||
            target.closest('.editor-post-publish-panel__header-publish-button')
          );
        }
    ...
    function makeStandalone() {
      ...
      const confirmMsg =
        L.makeStandaloneConfirm ||
        'Remove this date from the series and keep it as its own event? It will no longer update with the series.';
      if (!window.confirm(confirmMsg)) {
        return;
      }
      setMakingStandalone(true);
      window
        .fetch(L.standaloneRestUrl + postId, {
          method: 'POST',
          headers: {
            'X-WP-Nonce': L.restNonce || '',
          },
          ...
          customized
            ? el(
                ...
                el(
                  ...
                  null,
                  L.customContentHelp ||
                    'Changed fields keep your edits when the master updates. This date is still removed if it leaves the series.',
                ),
                el(
                  Button,
                  {
