(() => {
  // themes/baselayer/packages/baselayer-events/src/js/events.js
  (function() {
    "use strict";
    ...
    function expandOccurrences(rule, anchorStart, anchorEnd, horizon) {
      ...
      if (rule.freq === "weekly") {
        ...
        const start = /* @__PURE__ */ new Date(anchorStart + "T12:00:00");
        const n = (start.getDay() + 6) % 7;
        let cursor2 = new Date(start);
        cursor2.setDate(cursor2.getDate() - n);
        let weekIndex = 0;
        let guard2 = 0;
        while (guard2 < 5e3 && out.length < max) {
          ++guard2;
          const cursorYmd = ymdLocal(cursor2);
          if (cursorYmd > until) {
            break;
          }
      ...
      let guard = 0;
      while (cursor <= until && out.length < max && guard < 5e3) {
        ++guard;
    ...
    function RecurrenceModal(props) {
      ...
      const horizon = L.horizonDate || addDays(startDate || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), 365);
        ...
        el(
          ...
          el(
            ...
            el(
              ...
              !hasStartDate ? el("p", { className: "description" }, L.recurrenceNeedsDate || "Set a start date to create occurrence posts.") : el(
                Fragment,
                null,
                el(
                  "ul",
    ...
    function EventPanelContent() {
      ...
      const showStatus = (function() {
        ...
        const enabled = enabledMap && typeof enabledMap === "object" && Object.prototype.hasOwnProperty.call(enabledMap, postType) ? !!enabledMap[postType] : L.statusesEnabled !== false;
      ...
      const statusOptions = (function() {
        const raw = L.statusesByType && L.statusesByType[postType] || L.statuses || [];
      ...
      function revertToMaster() {
        ...
        setReverting(true);
        window.fetch(L.revertRestUrl + postId, {
          method: "POST",
          headers: {
      ...
      const recurrenceBlock = isOccurrence ? el(
        ...
        { className: "bl-event-recurring" },
        el("h3", { className: "bl-event-recurring__heading" }, L.recurringTitle || "Recurring"),
        rule ? summaryLines(rule).map(function(line, i) {
          return el("p", { key: "line-" + i, className: "bl-event-recurring__line" }, line);
        }) : el("p", { className: "description" }, L.notRepeating || "Not repeating"),
        !hasStartDate ? el("p", { className: "description bl-event-recurring__needs-date" }, L.recurrenceNeedsDate || "Set a start date to create occurrence posts.") : null,
        rule && hasStartDate && occurrenceCount != null ? el(
          "p",
          { className: "bl-event-recurring__count" },
          occurrenceCount === 1 ? (L.occurrenceLabel || "%d occurrence").replace("%d", "1") : (L.occurrencesLabel || "%d occurrences").replace("%d", String(occurrenceCount))
        ) : null,
        el(
          Button,
          {
        ...
        el(
          ...
          el(
            ...
            el("input", {
              ...
              value: endDraft,
              min: startDraft || startDate || void 0,
              disabled: dateFieldsDisabled,
              onChange: function(e) {
          ...
          showStatus ? el(
            ...
            el(SelectControl, {
              ...
              options: statusOptions.length ? statusOptions : [{ label: "None", value: "active" }],
            ...
            statusKey && statusKey !== "active" ? TextareaControl ? el(TextareaControl, {
  ...
  // themes/baselayer/packages/baselayer-events/src/js/events-meta.js
  (function() {
    "use strict";
    const wp = typeof window !== "undefined" ? window.wp : null;
    if (!wp || typeof baselayerEvents === "undefined") {
    ...
    function EventMetadataPanelContent() {
      ...
      Object.keys(config.groups).forEach(function(groupId) {
        ...
        if (lines.length) {
          summary.push({ id: groupId, title: group.title || groupId, lines });
        }
      });
      return el(
        PluginDocumentSettingPanel,
        ...
        el(
          ...
          summary.length ? summary.map(function(block) {
            return el(
              "div",
...
//# sourceMappingURL=events-editor.js.map
