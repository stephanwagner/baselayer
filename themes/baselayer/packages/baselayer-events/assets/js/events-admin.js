(() => {
  // themes/baselayer/packages/baselayer-events/src/js/event-occurrences.js
  (function() {
    "use strict";
    ...
    function ensureModal() {
      ...
      overlay.innerHTML = '<div class="bl-event-occurrences-modal__backdrop" data-bl-occ-close></div><div class="bl-event-occurrences-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="bl-event-occ-title"><div class="bl-event-occurrences-modal__header"><h2 id="bl-event-occ-title" class="bl-event-occurrences-modal__title"></h2><button type="button" class="bl-event-occurrences-modal__close" data-bl-occ-close aria-label="' + escapeAttr(L.closeLabel || "Close") + '">&times;</button></div><div class="bl-event-occurrences-modal__body"></div></div>';
    ...
    function openConfirm(message, onConfirm) {
      ...
      confirmOverlay.innerHTML = '<div class="bl-event-occurrences-confirm__backdrop" data-bl-occ-confirm-cancel></div><div class="bl-event-occurrences-confirm__dialog" role="dialog" aria-modal="true" aria-labelledby="bl-event-occ-confirm-title"><div class="bl-event-occurrences-confirm__header"><h2 id="bl-event-occ-confirm-title" class="bl-event-occurrences-confirm__title">' + escapeHtml(title) + '</h2><button type="button" class="bl-event-occurrences-confirm__close" data-bl-occ-confirm-cancel aria-label="' + escapeAttr(L.closeLabel || "Close") + '">&times;</button></div><div class="bl-event-occurrences-confirm__body"><p>' + escapeHtml(message) + '</p></div><div class="bl-event-occurrences-confirm__footer"><button type="button" class="button" data-bl-occ-confirm-cancel>' + escapeHtml(cancelText) + '</button><button type="button" class="button button-primary bl-event-occurrences-confirm__delete" data-bl-occ-confirm-ok>' + escapeHtml(deleteText) + "</button></div></div>";
    ...
    function showBodyError(bodyEl, message) {
      bodyEl.innerHTML = '<p class="bl-event-occurrences-modal__empty">' + escapeHtml(message || L.errorLabel || "Could not load occurrences.") + "</p>";
    }
    ...
    function renderList(bodyEl, items) {
      if (!items.length) {
        bodyEl.innerHTML = '<p class="bl-event-occurrences-modal__empty">' + escapeHtml(L.empty || "No upcoming occurrences.") + "</p>";
        return;
      }
      ...
      items.forEach(function(item) {
        ...
        if (deleted && item.start_date) {
          html += '<button type="button" class="button-link bl-event-occurrences-modal__restore" data-bl-occ-restore data-start-date="' + escapeAttr(item.start_date) + '">' + escapeHtml(L.restoreLabel || "Restore") + "</button>";
        } else if (!deleted) {
          if (edit) {
    ...
    function restoreOccurrence(btn) {
      ...
      }).then(refreshFromResponse).catch(function(err) {
        btn.disabled = false;
        const bodyEl = overlay && overlay.querySelector(".bl-event-occurrences-modal__body");
    ...
    function openModal(masterId, masterTitle) {
      ...
      window.fetch(L.restUrl + encodeURIComponent(String(masterId)), {
        method: "GET",
        headers: {
          "X-WP-Nonce": L.restNonce || "",
          Accept: "application/json"
...
//# sourceMappingURL=events-admin.js.map
