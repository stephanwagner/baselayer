 ...
 * @return bool
 */
function bl_webp_supported(): bool
{
 ...
 * @return int
 */
function bl_webp_quality(): int
{
	$q = function_exists('bl_config') ? bl_config('webp_quality') : null;
 ...
 * @return bool
 */
function bl_webp_convert_original(): bool
{
	return function_exists('bl_theme_feature_enabled') && bl_theme_feature_enabled('webp_convert_original');
}
 ...
 * @param bool   $lossless    Use lossless (PNG); otherwise lossy with bl_webp_quality().
 * @return bool True if WebP was created and source was removed.
 */
function bl_webp_convert_file(string $source_path, string $mime_type, bool $lossless): bool
{
	...
	$quality = $lossless ? 100 : bl_webp_quality();
	$ok = false;
 ...
 */
function bl_webp_convert_attachment_sizes(array $metadata, int $attachment_id): array
{
	if (!function_exists('bl_theme_feature_enabled') || !bl_theme_feature_enabled('webp')) {
		return $metadata;
	}
	if (!bl_webp_supported()) {
		return $metadata;
	}
	...
	if (!empty($metadata['sizes']) && is_array($metadata['sizes'])) {
		foreach ($metadata['sizes'] as $size_name => $size_data) {
			...
			if (!bl_webp_convert_file($source_path, $size_mime, $lossless)) {
				continue;
			}
	...
	if (bl_webp_convert_original() && file_exists($file)) {
		if (bl_webp_convert_file($file, $mime, $lossless_original)) {
...
add_filter('wp_generate_attachment_metadata', 'bl_webp_convert_attachment_sizes', 20, 2);
