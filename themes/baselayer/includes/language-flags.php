 ...
 */
function bl_language_data_path(string $filename): string
{
 ...
 */
function bl_iso639_language_catalog(): array
{
	static $catalog = null;
	if ($catalog !== null) {
		return $catalog;
	...
	$file = bl_language_data_path('iso639-catalog.json');
	if (!is_readable($file)) {
		$catalog = [];
		return $catalog;
	}
 ...
 */
function bl_iso639_language_catalog_by_id(): array
{
	$by_id = [];
	foreach (bl_iso639_language_catalog() as $entry) {
 ...
 */
function bl_language_flag_map(): array
{
	static $map = null;
	if ($map !== null) {
		return $map;
	...
	$file = bl_language_data_path('flag-map.json');
	if (!is_readable($file)) {
		$map = [];
		return $map;
	}
 ...
 */
function bl_language_flag_asset_path(string $lang_id): string
{
 ...
 */
function bl_language_flag_url(string $lang_id): string
{
	$relative = bl_language_flag_asset_path($lang_id);
	if ($relative === '') {
		return '';
	}

	return function_exists('bl_asset_url') ? bl_asset_url('/' . $relative) : '';
}

/**
 * Admin flag preview markup (Developer → Languages table).
 */
function bl_language_flag_preview_markup(string $lang_id = ''): string
{
	$url = bl_language_flag_url($lang_id);
	$hidden = $url === '' ? ' hidden' : '';
 ...
 */
function bl_language_flag_img(string $lang_id, string $label): string
{
	$url = bl_language_flag_url($lang_id);
	if ($url === '') {
		return '';
