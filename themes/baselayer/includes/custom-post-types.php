 ...
 * Registered CPTs are included in bl_theme_post_types() (theme-setup.php) and thus in SEO, post expirator, duplicate, etc.
 */
 ...
 */
function bl_content_type_wp_register_args(array $cfg): array
{
	$archive = bl_content_type_archive_from_cfg($cfg);
	$query = bl_content_type_query_from_cfg($cfg);
	$admin = bl_content_type_admin_from_cfg($cfg);
	...
	$args['_bl_has_menu_order'] = !empty($query['menu_order']);
	$args['_bl_orderby'] = isset($query['orderby']) && is_string($query['orderby']) ? $query['orderby'] : '';
	$args['_bl_order'] = isset($query['order']) && is_string($query['order']) ? $query['order'] : '';
 ...
 * @return array<string, mixed>
 */
function bl_content_type_archive_from_cfg(array $cfg): array
{
 ...
 * @return array<string, mixed>
 */
function bl_content_type_query_from_cfg(array $cfg): array
{
 ...
 * @return array<string, mixed>
 */
function bl_content_type_admin_from_cfg(array $cfg): array
{
 ...
 */
function bl_post_type_has_config_archive(?string $post_type = null): bool
{
	...
	if (!bl_content_type_enabled('post')) {
		return false;
	}

	$archive = bl_content_type_archive('post');
 ...
 */
function bl_post_archive_slug(): string
{
	if (!bl_post_type_has_config_archive('post')) {
		return '';
	}
 ...
 * @return array<string, mixed>
 */
function bl_post_register_args_from_config(array $args, string $post_type): array
{
	if ($post_type !== 'post') {
		return $args;
	}
...
add_filter('register_post_type_args', 'bl_post_register_args_from_config', 10, 2);

/**
 * Built-in `post` keeps `rewrite => false`; register archive URL rules explicitly.
 */
function bl_post_archive_register_rewrites(): void
{
	$slug = bl_post_archive_slug();
	if ($slug === '') {
		return;
...
add_action('init', 'bl_post_archive_register_rewrites', 20);
 ...
 */
function bl_post_archive_maybe_flush_rewrites(): void
{
	if (!bl_post_type_has_config_archive()) {
		delete_option('bl_post_archive_rewrite_sig');
	...
	if (get_option('bl_post_archive_rewrite_sig') === $sig) {
		return;
	}

	flush_rewrite_rules(true);
	update_option('bl_post_archive_rewrite_sig', $sig, false);
}

add_action('init', 'bl_post_archive_maybe_flush_rewrites', 99);
 ...
 */
function bl_cpt_merge_labels(string $post_type, array $provided_labels = []): array
{
	return array_merge(bl_cpt_default_labels($post_type, $provided_labels), $provided_labels);
}

/**
 * Translate plain English label strings from content-type config.
 * Must run before {@see bl_cpt_merge_labels()} so derived labels (e.g. “All %s”) use translated names.
 *
 * @param array<string, string> $strings
 * @return array<string, string>
 */
function bl_cpt_translate_config_label_strings(array $strings): array
{
	foreach ($strings as $key => $value) {
		if (!is_string($key) || !is_string($value) || $value === '') {
			continue;
		}
		$strings[$key] = __($value, 'baselayer');
	}
 ...
 */
function bl_apply_post_type_labels(string $post_type, array $labels): void
{
 ...
 */
function bl_post_apply_labels_from_config(): void
{
	if (!bl_content_type_enabled('post')) {
		return;
	}
	...
	$provided_labels = bl_cpt_translate_config_label_strings($provided_labels);
	$labels = bl_cpt_merge_labels('post', $provided_labels);
	$menu_label = bl_cpt_archive_menu_label('post');
	if ($menu_label !== '') {
		$labels['archives'] = $menu_label;
	}
	bl_apply_post_type_labels('post', $labels);
}

add_action('init', 'bl_post_apply_labels_from_config', 10);
 ...
 * @return void
 */
function bl_register_cpts(): void
{
	...
	foreach ($cpts as $post_type => $cfg) {
		...
		$args = array_merge($defaults, bl_content_type_wp_register_args($cfg));
		...
		$has_order = !empty($args['_bl_has_menu_order']);
		unset($args['_bl_has_menu_order'], $args['_bl_orderby'], $args['_bl_order']);
		...
		$provided_labels = bl_cpt_translate_config_label_strings($provided_labels);
		$args['labels'] = bl_cpt_merge_labels($post_type, $provided_labels);
		$menu_label = bl_cpt_archive_menu_label($post_type);
		if ($menu_label !== '') {
		...
		unset($args['icon']);
		$args['menu_icon'] = bl_cpt_menu_icon($menu_icon_value);
 ...
 */
function bl_cpt_menu_icon($icon): string
{
 ...
 */
function bl_cpt_taxonomy_map(): array
{
	$map = [];
	$sources = [];

	$post_cfg = bl_config_cpt('post');
	if (is_array($post_cfg) && bl_content_type_enabled('post', $post_cfg)) {
	...
	foreach ($sources as $object_type => $cfg) {
		...
		if (bl_content_type_uses_wp_categories($cfg)) {
			$taxonomies[] = 'category';
		}
 ...
 * @return array<string, string>
 */
function bl_cpt_default_taxonomy_labels(string $taxonomy, string $label, string $singular_label): array
{
	if ($label === '') {
	...
	return [
		...
		'search_items' => __('Search categories', 'baselayer'),
		'all_items' => __('All categories', 'baselayer'),
		'parent_item' => __('Parent category', 'baselayer'),
		'parent_item_colon' => __('Parent category:', 'baselayer'),
		'edit_item' => __('Edit category', 'baselayer'),
		'view_item' => __('View categories', 'baselayer'),
		'update_item' => __('Update category', 'baselayer'),
		'add_new_item' => __('Add new category', 'baselayer'),
		'new_item_name' => __('New category name', 'baselayer'),
	];
}
 ...
 * @return array<string, mixed>
 */
function bl_cpt_normalize_taxonomy_args(string $taxonomy, array $args): array
{
	$defaults = [
		'public' => true,
		'show_ui' => true,
	...
	if ($singular_label !== '') {
		$singular_label = __($singular_label, 'baselayer');
	}
	...
	$args['labels'] = array_merge(
		bl_cpt_default_taxonomy_labels($taxonomy, $label, $singular_label),
		$provided_labels
	);
 ...
 */
function bl_register_cpt_taxonomies(): void
{
	$taxonomies = bl_cpt_taxonomy_map();
	if ($taxonomies === []) {
		return;
	}
	...
	foreach ($taxonomies as $taxonomy => $entry) {
		...
		register_taxonomy(
			$taxonomy,
			$object_types,
			bl_cpt_normalize_taxonomy_args($taxonomy, is_array($entry['args']) ? $entry['args'] : [])
		);
	}
}
add_action('init', 'bl_register_cpt_taxonomies', 11);
...
add_action('init', function (): void {
	...
	if (!bl_content_type_uses_wp_categories($post_cfg)) {
 ...
 */
function bl_content_type_apply_wp_tags(): void
{
	foreach (bl_get_content_types() as $slug => $cfg) {
		if (!is_string($slug) || $slug === '' || !is_array($cfg) || !bl_content_type_enabled($slug, $cfg)) {
			continue;
		}
		...
		if (bl_content_type_uses_wp_tags($cfg)) {
...
add_action('init', 'bl_content_type_apply_wp_tags', 12);

/**
 * Convert inline SVG markup to data URI accepted by register_post_type menu_icon.
 */
function bl_cpt_svg_to_data_uri(string $svg): string
{
	$svg = bl_cpt_svg_apply_fill($svg, '#f3f1f1');
 ...
 */
function bl_cpt_svg_apply_fill(string $svg, string $fill): string
{
 ...
 */
function bl_cpt_admin_menu_icon_css(): string
{
	$cpts = bl_config_cpt('all');
	if (!is_array($cpts) || $cpts === []) {
		return '';
	...
	foreach ($cpts as $post_type => $args) {
		...
		$icon = bl_cpt_menu_icon($icon_value);
...
add_action('admin_head', function (): void {
	$css = bl_cpt_admin_menu_icon_css();
	if ($css === '') {
		return;
	}
	echo '<style id="bl-cpt-menu-icons">' . $css . '</style>';
}, 5);
 ...
 */
function bl_cpt_ordered_map(): array
{
	$cpts = bl_config_cpt('all');
	if (!is_array($cpts) || $cpts === []) {
		return [];
...
function bl_cpt_is_ordered(string $post_type): bool
{
	$map = bl_cpt_ordered_map();
 ...
 */
function bl_cpt_pre_get_posts_order(\WP_Query $query): void
{
	...
	if (bl_cpt_type($pt) === 'event') {
		return;
	}
	if (!is_array(bl_config_cpt($pt))) {
		return;
	}
	if ($pt === 'post' && !bl_post_type_has_config_archive('post')) {
		return;
	}
	$query_config = bl_content_type_query($pt);
...
add_action('pre_get_posts', 'bl_cpt_pre_get_posts_order', 15);

/**
 * Resolve current post type in admin contexts (including Quick Edit AJAX).
 */
function bl_cpt_current_admin_post_type(): string
{
...
add_action('pre_get_posts', function (\WP_Query $query): void {
	...
	if (!is_string($post_type) || $post_type === '' || !bl_cpt_is_ordered($post_type)) {
		return;
	}
...
add_action('save_post', function (int $post_id, \WP_Post $post): void {
	...
	if (!bl_cpt_is_ordered($post->post_type)) {
		return;
	}
 ...
 * @return array<string, string>
 */
function bl_cpt_admin_list_insert_order_column(array $columns): array
{
	$post_type = bl_cpt_current_admin_post_type();
	if ($post_type === '') {
		$post_type = 'post';
	}
	if (!bl_cpt_is_ordered($post_type)) {
		return $columns;
	}
	$reorder_label = __('Order', 'baselayer');
	$out = [];
	$inserted = false;
	foreach ($columns as $key => $label) {
		...
		if ($key === 'categories' || strncmp($key, 'taxonomy-', 9) === 0) {
			$out['bl_cpt_reorder'] = $reorder_label;
			$inserted = true;
		}
	}
	if (!$inserted) {
		...
		foreach ($out as $key => $label) {
			if ($key === 'date') {
				$with_fallback['bl_cpt_reorder'] = $reorder_label;
				$inserted = true;
			}
 ...
 */
function bl_cpt_admin_list_render_order_column(string $column, int $post_id): void
{
	if ($column !== 'bl_cpt_reorder') {
		return;
	}
	$post = get_post($post_id);
	if (!$post instanceof \WP_Post || !bl_cpt_is_ordered($post->post_type) || !current_user_can('edit_post', $post_id)) {
		return;
	}
	...
	$mk = function (string $dir, string $label, string $icon_svg_code, bool $disabled = false) use ($base, $post_id): string {
		if ($disabled) {
			return '<span class="bl-cpt-reorder-menu__action is-disabled" aria-disabled="true"><span class="bl-cpt-reorder-menu__icon" aria-hidden="true">' . $icon_svg_code . '</span><span>' . esc_html($label) . '</span></span>';
		}
		$url = add_query_arg([
			'bl_cpt_reorder' => $dir,
			'post_id'        => $post_id,
		], $base);
		$url = wp_nonce_url($url, 'bl_cpt_reorder_' . $dir . '_' . $post_id);
		return '<a class="bl-cpt-reorder-menu__action" href="' . esc_url($url) . '"><span class="bl-cpt-reorder-menu__icon" aria-hidden="true">' . $icon_svg_code . '</span><span>' . esc_html($label) . '</span></a>';
	};
	$order = (int) get_post_field('menu_order', $post_id);
	echo '<div class="bl-cpt-reorder-menu"><span class="bl-cpt-reorder-menu__order">' . esc_html((string) $order) . '</span>';
	echo '<button type="button" class="button button-small button-icon bl-cpt-reorder-menu__toggle" aria-expanded="false" aria-label="' . esc_attr__('Reorder', 'baselayer') . '" title="' . esc_attr__('Reorder', 'baselayer') . '"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor"><path d="M334.5-442.35Q324-452.7 324-468v-258l-80 80q-11 11-25.67 11-14.66 0-25.33-11-11-10.67-11-25.33Q182-686 193-697l142-142q5.4-5 11.7-7.5 6.3-2.5 13.5-2.5t13.5 2.5Q380-844 385-839l142 142q11 11 11 25t-11 25.48Q516-636 501.5-636T476-647l-80-79v258q0 15.3-10.29 25.65Q375.42-432 360.21-432t-25.71-10.35ZM586.3-113.5Q580-116 575-121L433-263q-11-10.91-10.5-25.45.5-14.55 11.5-26.03Q445-325 459.5-325t25.5 11l79 80v-258q0-15.3 10.29-25.65Q584.58-528 599.79-528t25.71 10.35Q636-507.3 636-492v258l80-80q11-11 25.67-11 14.66 0 25.33 11 11 10.67 11 25.33Q778-274 767-263L625-121q-5.4 5-11.7 7.5-6.3 2.5-13.5 2.5t-13.5-2.5Z"/></svg></button>';
	echo '<div class="bl-cpt-reorder-menu__popover" hidden>';
	echo $mk('up', __('Move up', 'baselayer'), '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M450-635.69 317.08-502.77q-8.31 8.31-20.89 8.5-12.57.19-21.27-8.5-8.69-8.69-8.69-21.08 0-12.38 8.69-21.07l179.77-179.77q10.85-10.85 25.31-10.85 14.46 0 25.31 10.85l179.77 179.77q8.3 8.3 8.5 20.88.19 12.58-8.5 21.27-8.7 8.69-21.08 8.69-12.38 0-21.08-8.69L510-635.69v351.84q0 12.77-8.62 21.39-8.61 8.61-21.38 8.61t-21.38-8.61q-8.62-8.62-8.62-21.39v-351.84Z"/></svg>', $is_first);
	echo $mk('down', __('Move down', 'baselayer'), '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M450-368.15V-720q0-12.77 8.62-21.38Q467.23-750 480-750t21.38 8.62Q510-732.77 510-720v351.85l132.92-132.93q8.31-8.3 20.89-8.5 12.57-.19 21.27 8.5 8.69 8.7 8.69 21.08 0 12.38-8.69 21.08L505.31-279.15q-10.85 10.84-25.31 10.84-14.46 0-25.31-10.84L274.92-458.92q-8.3-8.31-8.5-20.89-.19-12.57 8.5-21.27 8.7-8.69 21.08-8.69 12.38 0 21.08 8.69L450-368.15Z"/></svg>', $is_last);
	echo $mk('top', __('Move to top', 'baselayer'), '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M210-760q-12.77 0-21.38-8.62Q180-777.23 180-790t8.62-21.38Q197.23-820 210-820h540q12.77 0 21.38 8.62Q780-802.77 780-790t-8.62 21.38Q762.77-760 750-760H210Zm248.62 611.38Q450-157.23 450-170v-381.85l-92.92 92.93q-8.7 8.69-21.08 8.69-12.38 0-21.08-8.69-8.69-8.7-8.5-21.27.2-12.58 8.5-20.89l139.77-139.77q5.62-5.61 11.85-7.92 6.23-2.31 13.46-2.31t13.46 2.31q6.23 2.31 11.85 7.92l139.77 139.77q8.69 8.7 8.69 21.08 0 12.38-8.69 21.08-8.7 8.69-21.27 8.5-12.58-.2-20.89-8.5L510-551.85V-170q0 12.77-8.62 21.38Q492.77-140 480-140t-21.38-8.62Z"/></svg>', $is_first);
	echo $mk('bottom', __('Move to bottom', 'baselayer'), '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M210-140q-12.77 0-21.38-8.62Q180-157.23 180-170t8.62-21.38Q197.23-200 210-200h540q12.77 0 21.38 8.62Q780-182.77 780-170t-8.62 21.38Q762.77-140 750-140H210Zm256.54-171.23q-6.23-2.31-11.85-7.92L314.92-458.92q-8.3-8.31-8.5-20.89-.19-12.57 8.5-21.27 8.7-8.69 21.08-8.69 12.38 0 21.08 8.69L450-408.15V-790q0-12.77 8.62-21.38Q467.23-820 480-820t21.38 8.62Q510-802.77 510-790v381.85l92.92-92.93q8.31-8.3 20.89-8.5 12.57-.19 21.27 8.5 8.69 8.7 8.69 21.08 0 12.38-8.69 21.08L505.31-319.15q-5.62 5.61-11.85 7.92-6.23 2.31-13.46 2.31t-13.46-2.31Z"/></svg>', $is_last);
	echo '</div>';
	echo '</div>';
}
 ...
 */
add_action('init', function (): void {
	$ordered = bl_cpt_ordered_map();
	if ($ordered === []) {
		return;
	}
	foreach (array_keys($ordered) as $post_type) {
		add_filter('manage_' . $post_type . '_posts_columns', 'bl_cpt_admin_list_insert_order_column', 20);
		add_action('manage_' . $post_type . '_posts_custom_column', 'bl_cpt_admin_list_render_order_column', 20, 2);
		add_filter('manage_edit-' . $post_type . '_sortable_columns', function (array $columns): array {
			$columns['bl_cpt_reorder'] = 'menu_order';
			return $columns;
		});
	}
}, 20);
...
add_filter('request', function (array $vars): array {
	...
	if ($post_type === '' || !bl_cpt_is_ordered($post_type)) {
		return $vars;
	}
...
add_filter('admin_body_class', function (string $classes): string {
	...
	return $classes . ' bl-cpt-reorder-list';
});
...
add_action('admin_head', function (): void {
	...
	echo '<style>
	.bl-cpt-reorder-list table.wp-list-table.widefat,
	.bl-cpt-reorder-list table.wp-list-table.widefat tbody,
	.bl-cpt-reorder-list table.wp-list-table.widefat tr,
	.bl-cpt-reorder-list td.column-bl_cpt_reorder{overflow:visible;}
	.column-bl_cpt_reorder{width:115px; text-align: right;}
	th.column-bl_cpt_reorder a {display: flex; align-items: center; justify-content: flex-end;}
	td.bl_cpt_reorder.column-bl_cpt_reorder{white-space:nowrap;}
	.bl-cpt-reorder-menu{position:relative;display:inline-flex;align-items:center;gap:8px}
	.bl-cpt-reorder-menu__order{display:inline-block;min-width:18px;text-align:right;font-variant-numeric:tabular-nums;}
	.bl-cpt-reorder-menu__icon{line-height:0;display:inline-block;width:24px;height:24px;}
	.bl-cpt-reorder-menu__icon svg{width:24px;height:24px;}
	.bl-cpt-reorder-menu__popover{position:absolute;right:calc(26px + 4px);top:0;z-index:1000;display:flex;gap:4px;flex-direction:column;padding:6px;margin-left:6px;min-width:170px;background:#fff;border:1px solid #c3c4c7;border-radius:4px;box-shadow:0 2px 8px rgba(0,0,0,.15)}
	.bl-cpt-reorder-menu__popover[hidden]{display:none}
	.bl-cpt-reorder-menu__action{display:inline-flex;align-items:center;gap:6px;padding:4px 6px;border-radius:3px;color:#2271b1;text-decoration:none}
	.bl-cpt-reorder-menu__action:hover{background:#f0f6fc;color:#135e96}
	.bl-cpt-reorder-menu__action .dashicons{font-size:14px;width:14px;height:14px;line-height:14px}
	.bl-cpt-reorder-menu__action.is-disabled{color:#8c8f94;cursor:not-allowed;background:transparent}
	</style>';
});
...
add_action('admin_footer', function (): void {
	...
	(function(){
		...
		if(defaultOrderActive){
			var orderTh=document.querySelector("th.column-bl_cpt_reorder");
		...
		function closeAll(exceptMenu){
			document.querySelectorAll(".bl-cpt-reorder-menu").forEach(function(menu){
				if(exceptMenu && menu===exceptMenu) return;
				var pop=menu.querySelector(".bl-cpt-reorder-menu__popover");
				var btn=menu.querySelector(".bl-cpt-reorder-menu__toggle");
				if(pop){
					pop.hidden=true;
				}
		...
		document.addEventListener("click",function(e){
			var btn=e.target.closest(".bl-cpt-reorder-menu__toggle");
			if(btn){
				var menu=btn.closest(".bl-cpt-reorder-menu");
				var pop=menu?menu.querySelector(".bl-cpt-reorder-menu__popover"):null;
				if(!menu||!pop) return;
			...
			if(!e.target.closest(".bl-cpt-reorder-menu")){
				closeAll(null);
			}
		});
...
add_action('admin_init', function (): void {
	if (!is_admin() || empty($_GET['bl_cpt_reorder']) || empty($_GET['post_id'])) {
		return;
	}
	$dir = sanitize_key((string) wp_unslash($_GET['bl_cpt_reorder']));
	...
	if (!wp_verify_nonce($nonce, 'bl_cpt_reorder_' . $dir . '_' . $post_id)) {
		return;
	}
 ...
 * @return array<string, string>
 */
function bl_cpt_default_labels(string $post_type, array $labels = []): array
{
	...
	return [
		...
		'singular_name'         => $name,
		'menu_name'             => $menu_name,
		'add_new'               => __('Add New', 'baselayer'),
		'add_new_item'          => sprintf(__('Add New %s', 'baselayer'), $name),
		'edit_item'             => sprintf(__('Edit %s', 'baselayer'), $name),
		'new_item'              => sprintf(__('New %s', 'baselayer'), $name),
		'view_item'             => sprintf(__('View %s', 'baselayer'), $name),
		'view_items'            => sprintf(__('View %s', 'baselayer'), $plural),
		'search_items'          => sprintf(__('Search %s', 'baselayer'), $plural),
		'not_found'             => sprintf(__('No %s found.', 'baselayer'), $plural),
		'not_found_in_trash'    => sprintf(__('No %s found in Trash.', 'baselayer'), $plural),
		'all_items'             => sprintf(__('All %s', 'baselayer'), $plural),
		'archives'              => sprintf(__('%s Archives', 'baselayer'), $name),
		'attributes'            => sprintf(__('%s Attributes', 'baselayer'), $name),
		'insert_into_item'      => sprintf(__('Insert into %s', 'baselayer'), $name),
		'uploaded_to_this_item' => sprintf(__('Uploaded to this %s', 'baselayer'), $name),
		'filter_items_list'     => sprintf(__('Filter %s list', 'baselayer'), $plural),
		'items_list_navigation' => sprintf(__('%s list navigation', 'baselayer'), $plural),
		'items_list'            => sprintf(__('%s list', 'baselayer'), $plural),
	];
}
 ...
 */
function bl_archive_design(?string $post_type = null): string
{
	if ($post_type === null || $post_type === '') {
		$post_type = bl_archive_current_post_type();
	}
 ...
 */
function bl_cpt_type(?string $post_type = null): string
{
	if ($post_type === null || $post_type === '') {
 ...
 * @return string[]
 */
function bl_cpt_slugs_by_type(string $type): array
{
 ...
 * @return array<string, mixed>
 */
function bl_cpt_event_options(?string $post_type = null): array
{
	if ($post_type === null || $post_type === '') {
		$post_type = bl_archive_current_post_type();
	}
	if ($post_type === '' || bl_cpt_type($post_type) !== 'event') {
		return [];
	}
 ...
 */
function bl_cpt_text(string $key, ?string $post_type = null): string
{
	$aliases = [
		'archive_empty' => 'empty',
	];
 ...
 */
function bl_cpt_archive_menu_label(string $post_type): string
{
	if ($post_type === '') {
		return '';
	}
	...
	if (isset($texts['menu']) && is_string($texts['menu']) && $texts['menu'] !== '') {
		return __($texts['menu'], 'baselayer');
	}
	if (isset($texts['heading']) && is_string($texts['heading']) && $texts['heading'] !== '') {
		return __($texts['heading'], 'baselayer');
	}
 ...
 */
function bl_cpt_archive_label(string $post_type): string
{
	if ($post_type === '') {
		return '';
	}

	$heading = bl_cpt_text('heading', $post_type);
	if ($heading !== '') {
		return $heading;
	}
	...
	if ($obj instanceof \WP_Post_Type && isset($obj->labels->name) && is_string($obj->labels->name) && $obj->labels->name !== '') {
		return __($obj->labels->name, 'baselayer');
	}
 ...
 */
function bl_archive_heading(): string
{
	$post_type = bl_archive_current_post_type();
	if ($post_type !== '') {
		$heading = bl_cpt_archive_label($post_type);
		if ($heading !== '') {
			return $heading;
		}
	}
...
/**
 * `type` for the current archive listing (from {@see bl_archive_current_post_type()}).
 */
function bl_archive_cpt_type(): string
{
	$post_type = bl_archive_current_post_type();

	return $post_type !== '' ? bl_cpt_type($post_type) : 'default';
}

/**
 * Whether the main query is a CPT archive with `type` => `event`.
 */
function bl_is_event_archive(): bool
{
