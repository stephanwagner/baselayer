...
function bl_cpt_admin_list_render_order_column(string $column, int $post_id): void
{
	...
	echo '<button type="button" class="button bl-button -has-icon -icon-only -icon-swap-vertical bl-cpt-reorder-menu__toggle" aria-expanded="false" aria-label="' . esc_attr__('Reorder', 'baselayer') . '" title="' . esc_attr__('Reorder', 'baselayer') . '"></button>';
...
add_action('admin_head', function (): void {
	...
	.bl-cpt-reorder-menu__popover{position:absolute;right:calc(36px + 4px);top:0;z-index:1000;display:flex;gap:4px;flex-direction:column;padding:6px;margin-left:6px;min-width:170px;background:#fff;border:1px solid #c3c4c7;border-radius:4px;box-shadow:0 2px 8px rgba(0,0,0,.15)}
