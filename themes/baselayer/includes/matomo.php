 ...
 *
 * Filter {@see 'bl_matomo_skip_frontend_tracker'} receives the computed boolean — return true to force skip,
 * false to force load (e.g. test tracker on a dev box).
 */
function bl_matomo_skip_frontend_tracker(): bool
{
	$skip = false;
	...
	return (bool) apply_filters('bl_matomo_skip_frontend_tracker', $skip);
}
...
add_action('wp_head', function (): void {
	...
	if (bl_matomo_skip_frontend_tracker()) {
		return;
	}
	...
	$matomo_url = trim((string) get_option('baselayer_matomo_url', ''));
	$site_id = (int) get_option('baselayer_matomo_site_id', 1);
	$token = trim((string) get_option('baselayer_matomo_token_auth', ''));
	$custom_js = trim((string) get_option('baselayer_matomo_custom_js', ''));
