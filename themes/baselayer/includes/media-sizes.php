...
add_action('admin_init', function () {
	...
	foreach ($extra as $size) {
		...
		$name = _x($name_raw, 'Image size', 'baselayer');
		...
		add_settings_field(
			...
			function () use ($name, $opt_w, $opt_h, $default_w, $default_h) {
				...
				echo '<label for="' . esc_attr($opt_w) . '">' . esc_html__('Max Width', 'baselayer') . '</label> ';
				echo '<input name="' . esc_attr($opt_w) . '" type="number" step="1" min="0" id="' . esc_attr($opt_w) . '" value="' . esc_attr($w) . '" class="small-text" />';
				echo '<br />';
				echo '<label for="' . esc_attr($opt_h) . '">' . esc_html__('Max Height', 'baselayer') . '</label> ';
