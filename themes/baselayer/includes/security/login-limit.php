 ...
 */
function bl_login_limit_settings(): array
{
	$defaults = [
		'enabled' => false,
		'attempts' => 5,
 ...
 */
function bl_login_limit_authenticate($user, $username, $password)
{
	if (!bl_login_limit_settings()['enabled']) {
		return $user;
	}
	$ip = bl_client_ip();
	if ($ip === '') {
	...
	$key = 'baselayer_lla_lockout_' . md5($ip);
	...
	if ($lockout_until !== false && time() < (int) $lockout_until) {
		...
		$message = $minutes === 1
			? '<strong>' . esc_html__('Error', 'baselayer') . ':</strong> ' . esc_html__('Too many login attempts. Please try again in 1 minute.', 'baselayer')
			: '<strong>' . esc_html__('Error', 'baselayer') . ':</strong> ' . esc_html(sprintf(__('Too many login attempts. Please try again in %d minutes.', 'baselayer'), $minutes));
 ...
 * @return void
 */
function bl_login_limit_on_failed(string $username): void
{
	$s = bl_login_limit_settings();
	if (!$s['enabled']) {
		return;
	}
	$ip = bl_client_ip();
	...
	$key_attempts = 'baselayer_lla_' . md5($ip);
	$key_lockout = 'baselayer_lla_lockout_' . md5($ip);
	$now = time();
 ...
 * @return void
 */
function bl_login_limit_on_success(string $username, WP_User $user): void
{
	$ip = bl_client_ip();
	if ($ip === '') {
		return;
	}
	delete_transient('baselayer_lla_' . md5($ip));
}

if (bl_login_limit_settings()['enabled']) {
	add_filter('authenticate', 'bl_login_limit_authenticate', 999, 3);
	add_action('wp_login_failed', 'bl_login_limit_on_failed', 10, 1);
	add_action('wp_login', 'bl_login_limit_on_success', 10, 2);
}
