...
$aria_label = isset($aria_label) && is_string($aria_label) ? $aria_label : __('Posts pagination', 'baselayer');
...
$args = bl_paginate_links_args_for_wp_query($query, $overrides);
$links = bl_paginate_links_html($args);
if ($links === '') {
	return;
}

$default_nav_classes = ['pagination__wrapper', '-container-margin-m'];
...
<nav class="<?php echo esc_attr(implode(' ', $nav_classes)); ?>" aria-label="<?php echo esc_attr($aria_label); ?>">
	<div class="pagination__container">
		<?php
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- classes normalized in bl_paginate_links_apply_theme_classes().
		echo $links;
		?>
	</div>
</nav>
