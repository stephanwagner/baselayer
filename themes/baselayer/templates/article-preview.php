 ...
 *
 * Optional (via bl_render_template $data): id, url, classes, title_tag, range.
 */

$id = isset($id) ? $id : get_the_ID();
$url = (isset($url) && is_string($url) && $url !== '') ? $url : get_permalink();
$range = (isset($range) && is_string($range)) ? trim($range) : '';
if ($range === '' && function_exists('bl_event_format_range_text') && function_exists('bl_is_event_post_type')) {
	$post_type = get_post_type($id);
	if (is_string($post_type) && $post_type !== '' && bl_is_event_post_type($post_type)) {
		$range = bl_event_format_range_text((int) $id);
	}
}
