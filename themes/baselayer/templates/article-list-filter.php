...
$terms = bl_article_list_filter_terms($taxonomy);
if ($terms === []) {
	return;
}

$query_var = bl_article_list_filter_query_var($taxonomy, $filter_context);
if ($query_var === '') {
	return;
}

$tax_obj = get_taxonomy($taxonomy);
$all_label = __('All categories', 'baselayer');
...
if ($filter_context === 'block') {
	if ($form_action === '') {
		$form_action = bl_article_list_block_form_action();
	}
} else {
