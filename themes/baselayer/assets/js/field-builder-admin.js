(() => {
  ...
  var allTypes = [text_default, textarea_default, select_default, checkbox_default];
  ...
  function renderPresentation(container, data = {}) {
    ...
    const widthSelect = mark5(el("select", { className: "widefat" }), "presentation_width");
    [
      ["25", "25%"],
      ["50", "50%"],
      ["75", "75%"],
  ...
  function createFieldRow({ mode = "fields", data = {}, open = false } = {}) {
  ...
  function createShell(root, options = {}) {
    ...
    return {
      getSchema() {
        return Array.from(list.querySelectorAll("[data-bl-fb-field]")).map(
  ...
  function mount2(el2, options = {}) {
    if (!el2) {
      throw new Error("BlFieldBuilder.mount requires a container element");
    }
    ensureTypes();
    return createShell(el2, options);
  }
  var BlFieldBuilder = { mount: mount2 };
