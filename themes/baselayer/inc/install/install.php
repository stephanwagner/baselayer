...
require_once __DIR__ . '/child-theme.php';
  ...
  <div class="wrap">
    ...
    <?php } else { ?>
      ...
      <form class="baselayer__install-form" data-bl-install-form method="post" autocomplete="off">
        ...
        $testdata_notice_lines = function_exists('bl_install_testdata_notice_lines')
          ? bl_install_testdata_notice_lines()
          : [];
        ?>

        <table class="form-table" role="presentation">
          <tr>
            <th scope="row"><?= esc_html__('Create content', 'baselayer') ?></th>
            <td>
              <fieldset>
                ...
                <div data-bl-seed-mode-panel="test" style="<?= $content_seed_mode === 'test' ? '' : 'display: none;' ?>">
                  <div class="notice notice-info inline" style="margin: 0 0 12px;">
                    <p><strong><?= esc_html__('Test data will create:', 'baselayer') ?></strong></p>
                    <?php if ($testdata_notice_lines !== []) : ?>
                      <ul style="list-style: disc; margin-left: 1.25em;">
                        <?php foreach ($testdata_notice_lines as $line) : ?>
                          <li><?= esc_html($line) ?></li>
                        <?php endforeach; ?>
                      </ul>
                    <?php else : ?>
                      <p><?= esc_html__('All content types enabled, curated samples, plus a large set of posts, projects, and events (including past/future, timed, cancelled, recurring, and trash variants).', 'baselayer') ?></p>
                    <?php endif; ?>
                  </div>
                </div>
              </fieldset>
