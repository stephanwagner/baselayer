 ...
 */
const BL_MATOMO_STATS_CANONICAL_DAYS = 14;
const BL_MATOMO_STATS_CANONICAL_WEEKS = 9;
 ...
 */
const BL_MATOMO_DASHBOARD_VISITS_OPTION = 'bl_dashboard_matomo_quick_stats_v1';

/** Prevents stacking multiple wp-cron jobs for the same dashboard refresh. */
const BL_MATOMO_BACKGROUND_REFRESH_LOCK = 'bl_matomo_bg_refresh_lock';

/**
 * Cache TTL (seconds) for the Matomo bulk response and dashboard “staleness” checks. Default: one hour.
 * For local testing use e.g. `define( 'BL_MATOMO_STATS_CACHE_TTL', 30 );` in wp-config.php (above the “stop editing” line).
 */
if (!defined('BL_MATOMO_STATS_CACHE_TTL')) {
    define('BL_MATOMO_STATS_CACHE_TTL', HOUR_IN_SECONDS);
}

/**
 * Localized label for a visit count (Matomo nb_visits).
 */
function bl_format_visit_count(int $count): string
{
	return sprintf(
		_n('%1$s visit', '%1$s visits', $count, 'baselayer'),
 ...
 */
function bl_matomo_stats_cache_ttl_seconds(): int
{
    return max(1, (int) BL_MATOMO_STATS_CACHE_TTL);
}

/**
 * Settings
 */
function bl_dashboard_get_analytics_settings(): array
{
    return [
        'lineChart' => [
            'borderWidth' => 2,
 ...
 */
function bl_dashboard_can_access_statistics(): bool
{
    if (!is_user_logged_in()) {
        return false;
    }
    if (!function_exists('bl_theme_feature_enabled') || !bl_theme_feature_enabled('matomo')) {
        return false;
    }
...
add_action('admin_menu', function (): void {
    if (!bl_dashboard_can_access_statistics()) {
        return;
    }
    add_submenu_page(
        'index.php',
        __('Analytics', 'baselayer'),
        __('Analytics', 'baselayer'),
        'read',
        bl_dashboard_stats_page_slug(),
        'bl_render_dashboard_statistics_page'
    );
});
 ...
 */
function bl_dashboard_matomo_settings(): ?array
{
    if (!function_exists('bl_theme_feature_enabled') || !bl_theme_feature_enabled('matomo')) {
        return null;
    }
    ...
    if ($url === '' || $site_id <= 0 || $token === '') {
        $url = trim((string) get_option('baselayer_matomo_url', ''));
        $site_id = (int) get_option('baselayer_matomo_site_id', 1);
        $token = trim((string) get_option('baselayer_matomo_token_auth', ''));
    }
 ...
 */
function bl_dashboard_set_last_matomo_error(string $message): void
{
    $GLOBALS['bl_dashboard_last_matomo_error'] = $message;
}

function bl_dashboard_get_last_matomo_error(): string
{
    return (string) ($GLOBALS['bl_dashboard_last_matomo_error'] ?? '');
}

/**
 * Transient key for the Matomo day/week bulk response (must match bl_dashboard_get_matomo_daily_and_weekly).
 */
function bl_dashboard_matomo_bulk_cache_key(int $days, int $weeks): string
{
    $settings = bl_dashboard_matomo_settings();
    if ($settings === null) {
        return '';
    }
    ...
    return 'baselayer_matomo_day_week_' . md5(wp_json_encode([
        'url' => $settings['url'],
...
/**
 * True when the canonical bulk Matomo response is still in the transient cache (TTL {@see BL_MATOMO_STATS_CACHE_TTL}).
 * After expiry, dashboard AJAX should trigger a refresh so today/yesterday stay in sync with Matomo.
 */
function bl_dashboard_matomo_canonical_bulk_cache_valid(): bool
{
    $cache_key = bl_dashboard_matomo_bulk_cache_key(BL_MATOMO_STATS_CANONICAL_DAYS, BL_MATOMO_STATS_CANONICAL_WEEKS);
    if ($cache_key === '') {
        return false;
    }
 ...
 */
function bl_dashboard_matomo_dashboard_counts_need_full_refresh(): bool
{
    if (!function_exists('bl_matomo_statistics_refresh_full') || bl_dashboard_matomo_settings() === null) {
        return false;
    }
    if (!bl_dashboard_matomo_canonical_bulk_cache_valid()) {
        return true;
    }
    ...
    return (time() - $updated) >= bl_matomo_stats_cache_ttl_seconds();
}
 ...
 */
function bl_matomo_sync_dashboard_quick_stats_from_daily(array $daily): void
{
    $n = count($daily);
    if ($n === 0) {
    ...
    update_option(
        BL_MATOMO_DASHBOARD_VISITS_OPTION,
        [
            'today' => $today,
            'yesterday' => $yesterday,
 ...
 */
function bl_matomo_statistics_refresh_full(): array
{
    $key = bl_dashboard_matomo_bulk_cache_key(BL_MATOMO_STATS_CANONICAL_DAYS, BL_MATOMO_STATS_CANONICAL_WEEKS);
    if ($key !== '') {
        delete_transient($key);
    }

    return bl_matomo_get_statistics();
}
 ...
 * @return array<string, mixed>
 */
function bl_matomo_get_statistics(): array
{
    return bl_dashboard_get_matomo_daily_and_weekly(BL_MATOMO_STATS_CANONICAL_DAYS, BL_MATOMO_STATS_CANONICAL_WEEKS);
}

/** wp-cron: runs full Matomo refresh requested from the dashboard home (non-blocking). */
add_action('bl_matomo_background_statistics_refresh', static function (): void {
    try {
        if (function_exists('bl_matomo_statistics_refresh_full')) {
            bl_matomo_statistics_refresh_full();
        }
    } finally {
        delete_transient(BL_MATOMO_BACKGROUND_REFRESH_LOCK);
    }
});
 ...
 */
function bl_dashboard_matomo_site_calendar_dates(int $days): array
{
    $days = max(1, min(365, $days));
 ...
 */
function bl_dashboard_matomo_normalize_visits_summary_payload($payload): ?array
{
    if (!is_array($payload)) {
        return null;
    }
 ...
 */
function bl_dashboard_matomo_parse_visits_summary_90d($payload): array
{
    $empty = [
        'avg_time_on_site' => 0,
        'nb_visits' => 0,
    ...
    $stats = bl_dashboard_matomo_normalize_visits_summary_payload($payload);
    if ($stats === null) {
        return $empty;
    }
 ...
 * @return array{avg_time_on_site:int,nb_visits:int,nb_actions:int,nb_uniq_visitors:int,bounce_count:int}
 */
function bl_dashboard_default_visits_summary_90d(): array
{
    return [
        'avg_time_on_site' => 0,
        'nb_visits' => 0,
 ...
 * }
 */
function bl_dashboard_matomo_parse_visit_frequency_90d($payload): array
{
    $empty = [
        'nb_visits_new' => 0,
 ...
 * }
 */
function bl_dashboard_default_visit_frequency_90d(): array
{
    return [
        'nb_visits_new' => 0,
        'nb_visits_returning' => 0,
 ...
 * } $visit_frequency
 */
function bl_dashboard_render_visits_summary_90d_box(array $s, array $visit_frequency = []): void
{
    $s = array_merge(bl_dashboard_default_visits_summary_90d(), $s);
    $vf = array_merge(bl_dashboard_default_visit_frequency_90d(), $visit_frequency);
 ...
 */
function bl_dashboard_format_duration_seconds(int $seconds): string
{
    if ($seconds <= 0) {
        return '–';
    }
 ...
 */
function bl_dashboard_top_pages_max_hits(array $pages): int
{
    $m = 0;
 ...
 */
function bl_dashboard_render_top_pages_table(array $pages): void
{
    if ($pages === []) {
        return;
    }
 ...
 */
function bl_dashboard_render_top_referrers_table(array $referrers): void
{
    if ($referrers === []) {
        return;
    }
 ...
 */
function bl_dashboard_matomo_is_excluded_top_page_row(string $url, string $label): bool
{
 ...
 */
function bl_dashboard_matomo_parse_referrer_table_rows($payload): array
{
    if (!is_array($payload)) {
        return [];
    }
 ...
 * @return array<int, array{label:string,url:string,hits:int}>
 */
function bl_dashboard_matomo_merge_top_referrers(array $website_rows, array $engine_rows, int $limit = 10): array
{
    $by_url = [];
 ...
 * @param mixed $payload
 */
function bl_dashboard_matomo_parse_site_first_recording_ts($payload): ?int
{
    if (!is_array($payload)) {
        return null;
    }
 ...
 */
function bl_dashboard_default_alltime_summary(): array
{
    return [
        'nb_visits' => 0,
        'since_ts' => null,
 ...
 * Results are cached for {@see BL_MATOMO_STATS_CACHE_TTL} seconds (default one hour). Prefer {@see bl_matomo_get_statistics()} for the canonical bundle instead of arbitrary dimensions here.
 *
 * @return array{
 ...
 * }
 */
function bl_dashboard_get_matomo_daily_and_weekly(int $days = 7, int $weeks = 8): array
{
    $days = max(1, min(365, $days));
    $weeks = max(1, min(104, $weeks));
    $settings = bl_dashboard_matomo_settings();
    if ($settings === null || !function_exists('wp_remote_post')) {
        return [
            ...
            'pages' => [],
            'referrers' => [],
            'visits_summary_90d' => bl_dashboard_default_visits_summary_90d(),
            'visit_frequency_90d' => bl_dashboard_default_visit_frequency_90d(),
            'alltime_summary' => bl_dashboard_default_alltime_summary(),
        ];
    }

    $cache_key = bl_dashboard_matomo_bulk_cache_key($days, $weeks);
    ...
    if (!$bypass_cache) {
        ...
        if (
            ...
            if ($days === BL_MATOMO_STATS_CANONICAL_DAYS && $weeks === BL_MATOMO_STATS_CANONICAL_WEEKS) {
                bl_matomo_sync_dashboard_quick_stats_from_daily($cached['daily']);
            }
    ...
    if (is_wp_error($response)) {
        bl_dashboard_set_last_matomo_error($response->get_error_message());
    ...
    if ($status_code === 401) {
        ...
        bl_dashboard_set_last_matomo_error(__('Matomo: invalid or missing auth token.', 'baselayer') . ' ' . $msg);
        set_transient($cache_key, $empty, 5 * MINUTE_IN_SECONDS);
        return $empty;
    }
    if (is_array($data) && (($data['result'] ?? '') === 'error')) {
        bl_dashboard_set_last_matomo_error((string) ($data['message'] ?? __('Matomo API error', 'baselayer')));
        set_transient($cache_key, $empty, 5 * MINUTE_IN_SECONDS);
        return $empty;
    }
    if (!is_array($data) || !isset($data[0]) || !is_array($data[0])) {
        bl_dashboard_set_last_matomo_error(__('Unexpected Matomo response format.', 'baselayer'));
        set_transient($cache_key, $empty, 5 * MINUTE_IN_SECONDS);
        return $empty;
    }
    ...
    $map_rows = static function ($payload, int $daysOrWeeks, string $mode): array {
        ...
        $series = [];
        if ($mode !== 'week') {
            foreach (bl_dashboard_matomo_site_calendar_dates($daysOrWeeks) as $key) {
    ...
    if (is_array($pages_payload)) {
        ...
        foreach ($pages_payload as $row) {
            ...
            if (bl_dashboard_matomo_is_excluded_top_page_row($url, $label)) {
                continue;
            }
            $pages[] = [
                'label' => $label,
    ...
    $website_refs = bl_dashboard_matomo_parse_referrer_table_rows($data[7] ?? []);
    $engine_refs = bl_dashboard_matomo_parse_referrer_table_rows($data[8] ?? []);
    $out['referrers'] = bl_dashboard_matomo_merge_top_referrers($website_refs, $engine_refs, 10);

    $out['visits_summary_90d'] = bl_dashboard_matomo_parse_visits_summary_90d($data[6] ?? []);
    $out['visit_frequency_90d'] = bl_dashboard_matomo_parse_visit_frequency_90d($data[11] ?? []);

    $alltime_stats = bl_dashboard_matomo_parse_visits_summary_90d($data[10] ?? []);
    $out['alltime_summary'] = [
        'nb_visits' => (int) ($alltime_stats['nb_visits'] ?? 0),
        'since_ts' => bl_dashboard_matomo_parse_site_first_recording_ts($data[9] ?? []),
    ];

    bl_dashboard_set_last_matomo_error('');
    set_transient($cache_key, $out, bl_matomo_stats_cache_ttl_seconds());
    if ($days === BL_MATOMO_STATS_CANONICAL_DAYS && $weeks === BL_MATOMO_STATS_CANONICAL_WEEKS) {
        bl_matomo_sync_dashboard_quick_stats_from_daily($out['daily'] ?? []);
    }
 ...
 */
function bl_dashboard_format_week_date_range(DateTimeImmutable $week_start): string
{
 ...
 */
function bl_dashboard_analytics_row_is_today(string $date_ymd): bool
{
 ...
 */
function bl_dashboard_analytics_row_is_yesterday(string $date_ymd): bool
{
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date_ymd)) {
        return false;
    }
 ...
 */
function bl_dashboard_analytics_row_is_current_week(string $date_ymd): bool
{
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date_ymd)) {
        return false;
    }
 ...
 */
function bl_dashboard_stats_metric_maxima(array $rows): array
{
    $maxU = 0;
    $maxV = 0;
    $maxP = 0;
 ...
 * @return array{unique:int,visits:int,pageviews:int}
 */
function bl_dashboard_stats_metric_bar_widths(array $row, array $maxima): array
{
    $u = (int) ($row['unique'] ?? 0);
    $v = (int) ($row['visits'] ?? 0);
 ...
 * @param array{unique:int,visits:int,pageviews:int} $maxima
 */
function bl_dashboard_render_stats_metric_cells(array $row, array $maxima): void
{
    $w = bl_dashboard_stats_metric_bar_widths($row, $maxima);
    $cells = [
 ...
 */
function bl_dashboard_analytics_chart_date_label(int $timestamp): string
{
 ...
 */
function bl_dashboard_analytics_week_chart_axis_date_line(int $monday_timestamp): string
{
 ...
 */
function bl_dashboard_analytics_daily_axis_label(array $r)
{
    $date = (string) ($r['date'] ?? '');
    ...
    $ts = (int) $ts;
    $date_line = bl_dashboard_analytics_chart_date_label($ts);
    if (bl_dashboard_analytics_row_is_today($date)) {
        return [__('Today', 'baselayer'), $date_line];
    }
    if (bl_dashboard_analytics_row_is_yesterday($date)) {
        return [__('Yesterday', 'baselayer'), $date_line];
    }
 ...
 * @return array{0:string,1:string}|string
 */
function bl_dashboard_analytics_weekly_axis_label(array $r)
{
    $date = (string) ($r['date'] ?? '');
    $ts = $date !== '' ? strtotime($date . ' 00:00:00') : false;
    if (!$ts) {
    ...
    $date_line = bl_dashboard_analytics_week_chart_axis_date_line($monday->getTimestamp());
    if (bl_dashboard_analytics_row_is_current_week($date)) {
        return [
            __('This week', 'baselayer'),
            $date_line,
        ];
    }
 ...
 * @return array<string, mixed>
 */
function bl_dashboard_line_chart_config(array $labels, array $datasets): array
{
    return [
        'type' => 'line',
        'data' => [
...
{
    return admin_url('index.php?page=' . bl_dashboard_stats_page_slug());
}

/**
 * Analytics URL with cache bypass for administrators (skips Matomo transients).
 */
function bl_dashboard_statistics_reload_url(): string
{
    return add_query_arg('no_cache', '1', bl_dashboard_statistics_url());
}

function bl_render_dashboard_statistics_page(): void
{
    if (!bl_dashboard_can_access_statistics()) {
    ...
    $rows = $matomo_rows;
    $labels = array_map('bl_dashboard_analytics_daily_axis_label', $rows);
    ...
    $datasets = [
        [
            ...
            'data' => $unique,
            'borderWidth' => bl_dashboard_get_analytics_settings()['lineChart']['borderWidth'],
            'tension' => bl_dashboard_get_analytics_settings()['lineChart']['tension'],
            'pointRadius' => bl_dashboard_get_analytics_settings()['lineChart']['pointRadius'],
            'pointHoverRadius' => bl_dashboard_get_analytics_settings()['lineChart']['pointHoverRadius'],
            'fill' => true,
            'backgroundColor' => bl_dashboard_get_analytics_settings()['colors'][0]['transparent'],
            'borderColor' => bl_dashboard_get_analytics_settings()['colors'][0]['fill'],
            'fillColor' => bl_dashboard_get_analytics_settings()['colors'][0]['transparent'],
            'pointBackgroundColor' => bl_dashboard_get_analytics_settings()['colors'][0]['fill'],
        ],
        [
            ...
            'pointHoverRadius' => bl_dashboard_get_analytics_settings()['lineChart']['pointHoverRadius'],
            'fill' => true,
            'backgroundColor' => bl_dashboard_get_analytics_settings()['colors'][1]['transparent'],
            'borderColor' => bl_dashboard_get_analytics_settings()['colors'][1]['fill'],
            'fillColor' => bl_dashboard_get_analytics_settings()['colors'][1]['transparent'],
            'pointBackgroundColor' => bl_dashboard_get_analytics_settings()['colors'][1]['fill'],
        ],
        [
            ...
            'pointHoverRadius' => bl_dashboard_get_analytics_settings()['lineChart']['pointHoverRadius'],
            'fill' => true,
            'backgroundColor' => bl_dashboard_get_analytics_settings()['colors'][2]['transparent'],
            'borderColor' => bl_dashboard_get_analytics_settings()['colors'][2]['fill'],
            'fillColor' => bl_dashboard_get_analytics_settings()['colors'][2]['transparent'],
            'pointBackgroundColor' => bl_dashboard_get_analytics_settings()['colors'][2]['fill'],
        ],
    ];
    $line_chart_config = bl_dashboard_line_chart_config($labels, $datasets);

    $week_rows = array_slice($series['weekly'] ?? [], -8);
    $week_chart_config = [];
    if (!empty($week_rows)) {
        $week_labels = array_map('bl_dashboard_analytics_weekly_axis_label', $week_rows);
        ...
        $week_chart_config = bl_dashboard_line_chart_config($week_labels, $week_datasets);
    }
    ...
    $devices_chart_config = [
        ...
        'data' => [
            'labels' => [
                [__('Desktop', 'baselayer'), sprintf(__('%d%%', 'baselayer'), $device_pct($device_desktop))],
                [__('Mobile', 'baselayer'), sprintf(__('%d%%', 'baselayer'), $device_pct($device_mobile))],
                [__('Tablet', 'baselayer'), sprintf(__('%d%%', 'baselayer'), $device_pct($device_tablet))],
            ],
            'datasets' => [
                [
                    'data' => [
                    ...
                    'backgroundColor' => [
                        bl_dashboard_get_analytics_settings()['colors'][2]['transparent'],
                        bl_dashboard_get_analytics_settings()['colors'][0]['transparent'],
                        bl_dashboard_get_analytics_settings()['colors'][1]['transparent'],
                    ],
                    'borderColor' => [
                        bl_dashboard_get_analytics_settings()['colors'][2]['fill'],
                        bl_dashboard_get_analytics_settings()['colors'][0]['fill'],
                        bl_dashboard_get_analytics_settings()['colors'][1]['fill'],
                    ],
                    'borderWidth' => bl_dashboard_get_analytics_settings()['barChart']['borderWidth'],
                    'borderRadius' => [
                        'topLeft' => bl_dashboard_get_analytics_settings()['barChart']['borderRadius'],
                        'topRight' => bl_dashboard_get_analytics_settings()['barChart']['borderRadius'],
                    ],
                ],
            ],
        ],
        'options' => [
            'maxBarThickness' => bl_dashboard_get_analytics_settings()['barChart']['maxBarThickness'],
            'responsive' => true,
            'maintainAspectRatio' => false,
            'plugins' => [
    ...
    $visits_summary_90d = is_array($series['visits_summary_90d'] ?? null)
        ? array_merge(bl_dashboard_default_visits_summary_90d(), $series['visits_summary_90d'])
        : bl_dashboard_default_visits_summary_90d();
    ...
    <div class="wrap bl-analytics-page">
        ...
        ?>
            <div class="notice notice-error">
                <p><strong><?= esc_html__('Matomo error', 'baselayer') ?></strong></p>
                <p><?= esc_html($matomo_err) ?></p>
            </div>
        <?php
        endif;
        $matomo_settings = bl_dashboard_matomo_settings();
        $matomo_login_url = $matomo_settings ? $matomo_settings['url'] : '';
        $alltime = is_array($series['alltime_summary'] ?? null)
            ? array_merge(bl_dashboard_default_alltime_summary(), $series['alltime_summary'])
            : bl_dashboard_default_alltime_summary();
        ...
        <div class="notice inline bl-analytics-summary-notice">
            <div style="margin: 0; display: flex; flex-wrap: wrap; gap: 8px;">
                <div>
                    ...
                    <?= esc_html(bl_format_visit_count($alltime_visits)) ?>
                ...
                <?php if (current_user_can('manage_options')) : ?>
                    <span style="margin-left: auto;"><a href="<?= esc_url(bl_dashboard_statistics_reload_url()) ?>"><?= esc_html__('Clear cache', 'baselayer') ?></a></span>
                <?php endif; ?>
            </div>
        </div>
            ...
            <table class="widefat striped bl-stats-table" style="margin: 0;">
                ...
                <tbody>
                    <?php foreach (array_reverse($rows) as $r) :
                        ...
                        $is_today = $date !== '' && bl_dashboard_analytics_row_is_today($date);
                    ?>
                        <tr>
                            <td>
                                <?= esc_html($label) ?>
                    ...
                    <tbody>
                        <?php foreach (array_reverse($week_rows) as $r) :
                            ...
                            if ($ts) {
                                ...
                                $week_no = (int) $monday->format('W');
                                $week_label = sprintf(__('Week %d', 'baselayer'), $week_no);
                                $week_range = bl_dashboard_format_week_date_range($monday);
                            }
                            $is_current_week = $date !== '' && bl_dashboard_analytics_row_is_current_week($date);
                        ?>
                            <tr>
                                <td>
                                    <?php if ($week_range !== '') : ?>
                                ...
                                <?php bl_dashboard_render_stats_metric_cells($r, $weekly_metric_maxima); ?>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
