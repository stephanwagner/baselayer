...
function bl_dashboard_panel()
{
	...
	$system_url = admin_url('options-general.php?page=' . bl_developer_settings_page_slug('system') . '#bl-search-visibility');
	$security_url = admin_url('options-general.php?page=bl-developer-security');
...
?>
	<div class="bl-dashboard__panel">
					...
					<ul style="list-style: none; padding-left: 0; margin: 8px 0;">
						...
						?>
							<li style="margin-bottom: 8px">
								<code class="code bl-code-small"><?php echo esc_html($ip); ?></code> – <?php echo esc_html($block_hint); ?>
							</li>
						<?php endforeach; ?>
					</ul>
					<p style="margin-top: 8px;"><a href="<?php echo esc_url($security_url); ?>#bl-security-blocked-ips"><?php esc_html_e('Manage in Developer → Security', 'baselayer'); ?></a></p>
				</div>
		<?php
			endif;
		}
		...
		<div class="bl-dashboard__sections -flex">

			<div class="bl-dashboard__section -links">
				<div class="bl-dashboard__section-title"><?= esc_html__('Quick links', 'baselayer') ?></div>
				<ul class="bl-dashboard__section-list -limit">
					<?php if ($can_view_theme_settings) : ?>
						<li><a href="<?= esc_url(admin_url('options-general.php?page=bl-theme-settings')) ?>"><?= esc_html__('Theme settings', 'baselayer') ?></a></li>
					<?php endif; ?>
					<?php if ($is_developer) : ?>
			...
			<?php if (bl_theme_feature_enabled('matomo') && $can_view_stats) : ?>
				<div
					class="bl-dashboard__section -stats"
					data-bl-dashboard-stats
					data-bl-stats-cached="<?= $matomo_stats_cached ? '1' : '0' ?>"
					data-bl-expect-matomo-refresh="<?= $expect_matomo_refresh ? '1' : '0' ?>"
					data-bl-stats-loading="<?= $expect_matomo_refresh ? '1' : '0' ?>"
				>
					<div class="bl-dashboard__section-title">
						<?= esc_html__('Analytics', 'baselayer') ?>
						<span class="bl-dashboard__stats-spinner" aria-hidden="true"></span>
					</div>
					<ul class="bl-dashboard__section-list -limit">
						<li>
							<strong><?= esc_html__('Today', 'baselayer') ?>:</strong>
							<span data-bl-stat="today"><?= esc_html($matomo_today_html) ?></span>
						</li>
						<li>
							<strong><?= esc_html__('Yesterday', 'baselayer') ?>:</strong>
							<span data-bl-stat="yesterday"><?= esc_html($matomo_yesterday_html) ?></span>
						</li>
						<li>
		...
		<?php if (!empty($pinned_pages)) : ?>
			<div class="bl-dashboard__section -pinned -margin">
				<div class="bl-dashboard__section-title"><?= esc_html__('Pinned posts', 'baselayer') ?></div>
				<table class="bl-dashboard__section-table">
					<?php foreach ($pinned_pages as $pinned) : ?>
						...
						?>
						<tr>
							<td class="bl-dashboard__section-cell -type">
								<?= esc_html($pinned_type_label) ?>
							</td>
							<td class="bl-dashboard__section-cell -preview">
							...
							<td class="bl-dashboard__section-cell -title">
		...
		<?php if (!empty($scheduled)) : ?>
			<div class="bl-dashboard__section -margin">
				<div class="bl-dashboard__section-title"><?= esc_html__('Scheduled posts', 'baselayer') ?></div>
				<table class="bl-dashboard__section-table">
					<?php foreach ($scheduled as $item) : ?>
						<tr>
							<td class="bl-dashboard__section-cell -date">
								<?= esc_html(get_date_from_gmt((string) $item->post_date_gmt, get_option('date_format') . ' ' . get_option('time_format'))) ?>
							</td>
							<td class="bl-dashboard__section-cell -preview">
								<a href="<?= esc_url(get_permalink((int) $item->ID)) ?>" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h240q17 0 28.5 11.5T480-800q0 17-11.5 28.5T440-760H200v560h560v-240q0-17 11.5-28.5T800-480q17 0 28.5 11.5T840-440v240q0 33-23.5 56.5T760-120H200Zm560-584L416-360q-11 11-28 11t-28-11q-11-11-11-28t11-28l344-344H600q-17 0-28.5-11.5T560-800q0-17 11.5-28.5T600-840h200q17 0 28.5 11.5T840-800v200q0 17-11.5 28.5T800-560q-17 0-28.5-11.5T760-600v-104Z"/></svg></a>
							</td>
							<td class="bl-dashboard__section-cell -title">
		...
		<?php if (!empty($expiring_published)) : ?>
			<div class="bl-dashboard__section -margin">
				<div class="bl-dashboard__section-title"><?= esc_html__('Expiring posts', 'baselayer') ?></div>
				<table class="bl-dashboard__section-table">
					<?php
...
{
	echo '<div class="bl-dashboard__widget">';
	bl_dashboard_panel();
	echo '</div>';
}
...
(function () {
	document.addEventListener('DOMContentLoaded', function () {
		...
		function setStatsLoading(on) {
			wrap.setAttribute('data-bl-stats-loading', on ? '1' : '0');
		}
		...
		function applyStats(res) {
			...
			var d = res.data;
			var t = wrap.querySelector('[data-bl-stat="today"]');
			var y = wrap.querySelector('[data-bl-stat="yesterday"]');
			if (d.pending) {
				if (t && d.today != null) {
			...
			wrap.setAttribute('data-bl-stats-cached', '1');
			return true;
		}
		...
		function fetchStats() {
			var expectRefresh = wrap.getAttribute('data-bl-expect-matomo-refresh') === '1';
			if (expectRefresh) {
				setStatsLoading(true);
			}
			fetchStatsBody().then(function (res) {
...
JS;
	wp_add_inline_script('bl-dashboard-matomo', $inline);
}
