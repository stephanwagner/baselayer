...
function bl_breadcrumbs(array $args = []): string
{
    ...
    elseif (is_single()) {
        ...
        if ($post_type === 'post') {
            if (function_exists('bl_post_type_has_config_archive') && bl_post_type_has_config_archive('post')) {
                $label = function_exists('bl_cpt_archive_label') ? bl_cpt_archive_label('post') : '';
        ...
        elseif ($post_type !== 'page') {
            ...
            if ($obj && ! empty($obj->has_archive)) {
                $label = function_exists('bl_cpt_archive_label') ? bl_cpt_archive_label($post_type) : (string) $obj->labels->name;
                if ($label !== '') {
                    $items[] = [
                        'label' => $label,
    ...
    elseif (is_search()) {
        $items[] = [
            'label' => __('Search results', 'baselayer'),
            'url' => null,
        ];
    }
    ...
    elseif (is_archive()) {
        ...
        if (is_post_type_archive()) {
            ...
            if ($pto instanceof \WP_Post_Type && function_exists('bl_cpt_archive_label')) {
                $label = bl_cpt_archive_label($pto->name);
            }
        }
        if ($label === '') {
