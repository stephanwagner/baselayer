 ...
 */
function bl_image_with_placeholder_resolve($image_id, $size = 'large'): array
{
	$id = absint($image_id);
	...
	$fallback = (int) get_option('baselayer_feature_image_fallback', 0);
	...
	if (is_file($static) && is_readable($static)) {
		...
		return [
			'url' => bl_asset_url('/img/placeholder.webp'),
			'width'  => $dimensions[0],
			'height' => $dimensions[1],
			'attachment_id' => 0
		];
...
/**
 * Image URL with placeholder fallbacks (see {@see bl_image_with_placeholder_resolve()}).
 *
 ...
 */
function bl_image_with_placeholder_url($image_id, $size = 'large'): string
{
	return bl_image_with_placeholder_resolve($image_id, $size)['url'];
}

/**
 * `<img>` for the resolved image. Media Library results (featured image or theme “fallback featured image” option)
 * go through {@see bl_img()} (`srcset` / `sizes` / dimensions). The on-disk `assets/img/placeholder.webp` branch is
 ...
 */
function bl_image_with_placeholder($image_id, $size = 'large', array $attr = []): string
{
	$resolved = bl_image_with_placeholder_resolve($image_id, $size);
	if ($resolved['url'] === '') {
		return '';
	}
	...
	if ($attachment_id > 0) {
		unset($attr['src']);
		return bl_img($attachment_id, $size, $attr);
	}
