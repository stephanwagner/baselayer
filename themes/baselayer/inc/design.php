 ...
 */
function bl_extract_scss_root_block(string $scss): string
{
 ...
 * @return string
 */
function bl_scss_line_comments_to_css_block(string $scss): string
{
	$len = strlen($scss);
	$out = '';
	$i = 0;
 ...
 */
function bl_theme_json_layout_sizes(): array
{
	$layout = bl_config('layout');
	if (!is_array($layout)) {
		$layout = [];
	}
 ...
 */
function bl_theme_json_root_spacing(): array
{
	$layout = bl_config('layout');
	if (!is_array($layout)) {
		$layout = [];
 ...
 */
function bl_theme_json_color_settings(): array
{
	$options = bl_config('color_options');
	if (!is_array($options)) {
		$options = [];
	}
 ...
 * @return array<string, bool>
 */
function bl_theme_json_typography_settings(): array
{
	$options = bl_config('typography_options');
	if (!is_array($options)) {
		$options = [];
	}
 ...
 */
function bl_output_custom_css(): void
{
	$css = get_option('baselayer_custom_css', '');
	if ($css === '') {
		return;
	}
	...
	echo '<style id="baselayer-custom-css">' . trim($css) . '</style>' . "\n";
}
