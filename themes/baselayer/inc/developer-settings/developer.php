...
add_action('admin_menu', function () use ($bl_developer_tab, $bl_developer_page_slug) {
	...
	add_submenu_page(
		...
		'bl_render_developer_cheatsheet',
		bl_developer_tab_position($bl_developer_tab)
	);
}, 20);

function bl_render_developer_cheatsheet(): void
{
	if (!current_user_can('manage_options')) {
...
?>
	<div class="wrap">
		...
		<?php
		if (function_exists('bl_developer_render_system_info_panel')) {
		...
		<div class="fs-page-settings-form" style="margin-top: 0;">
			...
			<table class="widefat -large-padding striped helpers-table__table -columns-2">
				<tbody>
					<tr>
						<td>
							<strong><?= esc_html__('Simulate client IP', 'baselayer') ?></strong><br>
			...
			<h2 class="title" style="margin-top: 0;"><?= esc_html__('Helpers', 'baselayer') ?></h2>
			<p class="description"><?= esc_html__('Common helper functions and utilities for templates, theme code, and frontend scripts.', 'baselayer') ?></p>

			<table class="widefat -large-padding striped helpers-table__table">
				<tbody>
					<tr>
						<td>
							<strong><?= esc_html__('Asset URL', 'baselayer') ?></strong><br>
					...
					<tr>
						<td>
							<strong><?= esc_html__('Inline SVG Code', 'baselayer') ?></strong><br>
					...
					<tr>
						<td>
							<strong><?= esc_html__('Image HTML', 'baselayer') ?></strong><br>
					...
					<tr>
						<td>
							<strong><?= esc_html__('Image with placeholder', 'baselayer') ?></strong><br>
					...
					<tr>
						<td>
							<strong><?= esc_html__('Config Variable', 'baselayer') ?></strong><br>
					...
					<tr>
						<td>
							<strong><?= esc_html__('Breadcrumbs', 'baselayer') ?></strong><br>
  ...
  \'separator_html\' => \'<b>→</b>\',
					...
					<tr>
						<td>
							<strong><?= esc_html__('Modal', 'baselayer') ?></strong><br>
			...
			<div class="fs-admin-group -has-margin">
				...
				$icon_ui_strings = function_exists('bl_icon_ui_strings') ? bl_icon_ui_strings() : [];
				$button_icon = 'medal';
				...
				$inline_before_label = __('Open link', 'baselayer');
				...
				$inline_after_label = __('Read more', 'baselayer');
				...
				$svg_icon_path = function_exists('bl_icon_svg_asset_path')
					...
					: '/icons/' . $svg_icon . '.svg';
				$svg_php_code = "bl_svg_code('" . $svg_icon_path . "', ['class' => 'my-class']);";
				$svg_markup = bl_svg_code($svg_icon_path);
				?>
				...
				<p class="description"><?= wp_kses(
					sprintf(
						...
						__('You can find more icons in the %s.', 'baselayer'),
						'<a href="' . esc_url('https://fonts.google.com/icons?icon.style=Rounded') . '" target="_blank" rel="noopener noreferrer">' . esc_html__('Material Symbols library', 'baselayer') . '</a>'
					),
