 ...
 * in bl_developer_perf_scale_defaults() (and optionally the option ids below).
 */

/**
 * Performance scale band labels (universal).
 */
function bl_developer_perf_band_labels(): array
{
	return [
		__('Excellent', 'baselayer'),
		__('Good', 'baselayer'),
		__('Acceptable', 'baselayer'),
		__('Heavy', 'baselayer'),
		__('Problematic', 'baselayer'),
	];
}
 ...
 */
function bl_developer_perf_scale_defaults(): array
{
	return [
...
/**
 * Get performance scale config for a metric. Edit bl_developer_perf_scale_defaults() to adjust boundaries.
 * Normalizes to max (last value) + boundaries (first 4) for internal use.
 */
function bl_developer_perf_scale_config(string $metric): array
{
	$defaults = bl_developer_perf_scale_defaults();
 ...
 */
function bl_developer_perf_db_server(): ?array
{
	global $wpdb;
 ...
 */
function bl_developer_perf_object_cache_label(): string
{
 ...
 */
function bl_developer_perf_xdebug_enabled(): bool
{
 ...
 */
function bl_developer_perf_metrics(): array
{
 ...
 */
function bl_developer_perf_format_time(float $seconds): array
{
 ...
 * @param array|null  $labels     Optional; default bl_developer_perf_band_labels().
 * @return string Band label.
 */
function bl_developer_perf_band(float $value, array $boundaries, ?array $labels = null): string
{
	$labels = $labels ?? bl_developer_perf_band_labels();
	foreach (array_values($boundaries) as $i => $b) {
		if ((float) $value <= (float) $b) {
			return $labels[$i] ?? $labels[4];
		}
	}
	return $labels[4] ?? __('Problematic', 'baselayer');
}
 ...
 */
function bl_developer_perf_scale_html(float $value, $metric_or_config, array $opts = []): string
{
	$config = is_array($metric_or_config)
		? $metric_or_config
		: bl_developer_perf_scale_config((string) $metric_or_config);
	$max = (float) ($config['max'] ?? 250);
	$boundaries = array_values($config['boundaries'] ?? [20, 50, 100, 200]);
	$labels = bl_developer_perf_band_labels();
	$label = bl_developer_perf_band($value, $boundaries, $labels);
 ...
 */
function bl_developer_perf_show_in_admin_bar(): bool
{
	return get_option('baselayer_perf_admin_bar', '1') === '1';
}

/**
 * Current request client IP (for display and guest panel allowlist).
 */
function bl_developer_perf_current_ip(): string
{
...
function bl_developer_perf_show_nginx_purge_in_admin_bar(): bool
{
	if (!function_exists('bl_config')) {
		return true;
	}
	$enabled = bl_config('nginx_site_cache.enabled');
	if ($enabled !== null) {
		return (bool) $enabled;
	}
	// Backward compat with old flat config key.
	$legacy = bl_config('nginx_cache_purge_show_in_admin_bar');
 ...
 */
function bl_developer_perf_panel_guest_visible(): bool
{
	if (is_user_logged_in()) {
		return false;
	}
	if (get_option('baselayer_perf_panel_guest', '0') !== '1') {
		return false;
	}
	$current = bl_developer_perf_current_ip();
	if ($current === '') {
		return false;
	}
	$allowed = get_option('baselayer_perf_panel_guest_ips', '');
	if ($allowed === '') {
		return false;
	}
 ...
 * Endpoint: /wp-admin/admin-post.php?action=bl_purge_cache&_wpnonce=...
 */
add_action('admin_post_bl_purge_cache', function (): void {
	if (!current_user_can('manage_options')) {
		wp_die('Unauthorized');
	}
	check_admin_referer('bl_purge_cache');
	...
	if ($uid > 0) {
		bl_admin_notice(
			...
			$ok ? 'success' : 'error',
			$ok
				? __('Cache purged.', 'baselayer')
				: __('Cache purge finished with issues.', 'baselayer')
		);
	}
...
add_action('admin_bar_menu', function ($admin_bar): void {
	$show_perf = bl_developer_perf_show_in_admin_bar();
	...
	if (current_user_can('manage_options') && bl_developer_perf_show_nginx_purge_in_admin_bar()) {
	...
	if ($show_perf && (!function_exists('bl_is_developer_user') || !bl_is_developer_user((int) get_current_user_id()))) {
		return;
	}
	$perf = bl_developer_perf_metrics();
	$scale = function ($value, $metric, $unit = '', $aria = '') {
		return function_exists('bl_developer_perf_scale_html')
			? bl_developer_perf_scale_html((float) $value, $metric, ['compact' => true, 'unit' => $unit, 'aria_label_metric' => $aria])
			: '';
	};
	...
	if ($show_perf) {
		...
		$admin_bar->add_node([
			...
			'title'  => (function () use ($perf, $admin_bar_row): string {
				$t = bl_developer_perf_format_time((float) $perf['time']);
				return $admin_bar_row(__('Execution time', 'baselayer'), (string) $t['value'] . (string) $t['unit'], (float) $perf['time'], 'time', 's');
			})(),
		]);

		$admin_bar->add_node([
			'parent' => 'bl_wp_perf',
			'id'     => 'bl_wp_perf_memory',
			'title'  => $admin_bar_row(__('Peak memory', 'baselayer'), (string) $perf['memory'] . ' MB', (float) $perf['memory'], 'memory', ' MB'),
		]);

		$admin_bar->add_node([
			'parent' => 'bl_wp_perf',
			'id'     => 'bl_wp_perf_queries',
			'title'  => $admin_bar_row(__('DB queries', 'baselayer'), (string) $perf['queries'], (float) $perf['queries'], 'queries', ''),
		]);

		$admin_bar->add_node([
			'parent' => 'bl_wp_perf',
			'id'     => 'bl_wp_perf_hooks',
			'title'  => $admin_bar_row(__('Hooks fired', 'baselayer'), (string) $perf['hooks'], (float) $perf['hooks'], 'hooks', ''),
		]);
	}
}, 999);
 ...
 */
function bl_developer_perf_pinned_panel_render(): void
{
	if (is_user_logged_in()) {
		return;
	}
	if (!function_exists('bl_developer_perf_panel_guest_visible') || !bl_developer_perf_panel_guest_visible()) {
		return;
	}
	$perf = bl_developer_perf_metrics();
	$panel_css = '
		...
		.bl-perf-pinned-panel__minimize { font-size: 11px; color: #2271b1; cursor: pointer; }
		.bl-perf-pinned-panel__minimize:hover { color: #1c548a; }
		.bl-perf-pinned-panel__body { padding: 8px; }
		.bl-perf-pinned-panel__table { border-collapse: collapse; margin: 0; table-layout: fixed; }
		.bl-perf-pinned-panel__table th,
		.bl-perf-pinned-panel__table td { min-width: 75px; padding: 6px 16px 6px 0; vertical-align: middle; border-bottom: 1px solid #dcdcde; }
		.bl-perf-pinned-panel__table th:last-child,
		.bl-perf-pinned-panel__table td:last-child { padding-right: 0; }
		.bl-perf-pinned-panel__table tbody tr:last-child th,
		.bl-perf-pinned-panel__table tbody tr:last-child td { border-bottom: 0; }
		.bl-perf-pinned-panel__table thead th { font-weight: 600; text-align: left; font-size: 11px; }
		.bl-perf-pinned-panel__table thead th.bl-perf-pinned-panel__th-avg { white-space: nowrap; }
		.bl-perf-pinned-panel__avg-header { display: flex; align-items: center; gap: 6px; }
		.bl-perf-pinned-panel__pages { color: #646970; font-weight: normal; white-space: nowrap; }
		.bl-perf-pinned-panel__table tbody th { font-weight: normal; color: #646970; text-align: left; }
		.bl-perf-pinned-panel__table td.bl-perf-pinned-panel__avg-cell { white-space: nowrap; }
		.bl-perf-pinned-panel__table td.bl-perf-pinned-panel__scale-cell { vertical-align: middle; text-align: right; }
		.bl-perf-pinned-panel__table .bl-perf-clear-history { margin-left: auto; cursor: pointer; color: #2271b1; font-weight: normal; }
		.bl-perf-pinned-panel__table .bl-perf-clear-history:hover { color: #1c548a; }
		.bl-perf-pinned-panel .bl-perf-pinned-panel__scale-cell .bl-perf-scale { display: inline-block; margin-left: 0; vertical-align: middle; line-height: 1; }
	...
	';
	$scale_config = [
		'time'    => bl_developer_perf_scale_config('time'),
		'memory'  => bl_developer_perf_scale_config('memory'),
		'queries' => bl_developer_perf_scale_config('queries'),
		'hooks'   => bl_developer_perf_scale_config('hooks'),
		'labels'  => bl_developer_perf_band_labels(),
		'i18n'    => [
			'pages_one'       => __('1 page', 'baselayer'),
			'pages_many'      => __('%s pages', 'baselayer'),
			'average'         => __('Average', 'baselayer'),
			'clear'           => _x('Clear', 'performance panel history', 'baselayer'),
			'no_data'         => __('No data yet.', 'baselayer'),
			'execution_time'  => __('Execution time', 'baselayer'),
			'peak_memory'     => __('Peak memory', 'baselayer'),
			'db_queries'      => __('DB queries', 'baselayer'),
			'hooks_fired'     => __('Hooks fired', 'baselayer'),
		],
	];
...
?>
	...
	<script>
		(function() {
			...
			var content = panel.querySelector('.bl-perf-pinned-panel__content');
			var tab = panel.querySelector('.bl-perf-pinned-panel__tab');
			var MINIMIZED_KEY = 'bl_perf_minimized';
...
add_action('wp_footer', 'bl_developer_perf_pinned_panel_render', 20);
