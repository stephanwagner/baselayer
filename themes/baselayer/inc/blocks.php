...
add_action('init', function () {
    ...
    register_block_style('core/image', [
        'name'  => 'framed',
        'label' => __('Framed', 'baselayer'),
    ]);

    register_block_style('core/image', [
        'name'  => 'round',
        'label' => __('Round', 'baselayer'),
    ]);

    register_block_style('core/image', [
        'name'  => 'sharp',
        'label' => __('Sharp', 'baselayer'),
    ]);
}, 100);
...
add_filter('render_block', function ($content, $block) {
    ...
    $visible_text = bl_button_visible_label($attrs['text'] ?? '');

    if ($visible_text === '' && preg_match('/<a\b[^>]*\bwp-block-button__link\b[^>]*>(.*?)<\/a>/s', $content, $matches)) {
        $visible_text = bl_button_visible_label($matches[1]);
    }
 ...
 */
function bl_button_visible_label(string $text): string
{
