 ...
 */
class BL_Walker_Nav_Menu extends Walker_Nav_Menu
{
	/** @var array<int, string> */
	private array $submenu_ids_by_depth = [];

	private function bl_menu_scope($args): string
	{
		$menu_id = '';
	...
	private function bl_build_submenu_id($args, int $item_id): string
	{
		$scope = $this->bl_menu_scope($args);
		if ($item_id > 0) {
	...
	public function start_lvl(&$output, $depth = 0, $args = null): void
	{
		...
		$submenu_id = $this->submenu_ids_by_depth[$depth] ?? $this->bl_build_submenu_id($args, 0);
	...
	public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0): void
	{
		...
		$submenu_id = '';
		if ($has_children) {
			$submenu_id = $this->bl_build_submenu_id($args, (int) $item->ID);
		...
		if ($has_children) {
			$toggle_label = sprintf(
				...
				__('Toggle submenu for %s', 'baselayer'),
 ...
 */
function bl_nav_menu(array $args = []): void
{
	...
	if (empty($args['walker']) && class_exists('BL_Walker_Nav_Menu')) {
		$args['walker'] = new BL_Walker_Nav_Menu();
	}
	wp_nav_menu($args);
}
