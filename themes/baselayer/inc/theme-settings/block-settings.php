...
add_action('admin_init', function () {
	...
	if (empty($_POST['baselayer_save_block_settings']) || empty($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'baselayer_save_block_settings')) {
		return;
	}
	if (function_exists('bl_admin_can_access') && !bl_admin_can_access('theme_settings_blocks')) {
		return;
	}

	$posted = function_exists('bl_block_settings_parse_posted_settings')
		? bl_block_settings_parse_posted_settings()
		: [];

	$complete = [];
	foreach (array_keys(bl_block_settings_get_all()) as $block_name) {
		$complete[$block_name] = $posted[$block_name] ?? [];
	}

	$sanitized = function_exists('bl_sanitize_block_settings')
		? bl_sanitize_block_settings($complete)
		: [];
	...
	update_option(BL_BLOCK_SETTINGS_OPTION, $sanitized);

	if (function_exists('bl_block_variation_settings_parse_posted_settings') && function_exists('bl_sanitize_block_variation_settings')) {
		$variation_posted = bl_block_variation_settings_parse_posted_settings();
		$variation_sanitized = bl_sanitize_block_variation_settings($variation_posted);
		update_option(BL_BLOCK_VARIATION_SETTINGS_OPTION, $variation_sanitized);
	}

	set_transient('baselayer_blocks_saved', '1', 30);
	wp_safe_redirect(bl_theme_settings_url_with_tab('blocks'));
	exit;
}, 1);
...
add_action('admin_enqueue_scripts', function ($hook_suffix) {
	...
	if (!function_exists('bl_theme_settings_current_tab') || bl_theme_settings_current_tab() !== 'blocks') {
		return;
	}

	if (!current_user_can('manage_options') || (function_exists('bl_admin_can_access') && !bl_admin_can_access('theme_settings_blocks'))) {
		return;
	}
	...
	if (function_exists('get_block_editor_server_block_settings')) {
		...
		if (class_exists('WP_Block_Editor_Context') && function_exists('get_block_categories')) {
			$block_editor_context = new WP_Block_Editor_Context(['name' => 'baselayer/block-settings']);
			wp_add_inline_script(
				'wp-blocks',
				sprintf(
	...
	if (function_exists('bl_block_settings_admin_config')) {
		wp_localize_script(
			'baselayer-block-settings',
			'baselayerBlockSettingsAdmin',
			bl_block_settings_admin_config()
		);
	}
