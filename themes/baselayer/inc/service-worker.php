...
/**
 * Front-end service worker: registration data for baselayer.js + PHP route for the worker script.
 *
 ...
 */
function bl_service_worker_scope(): string
{
 ...
 */
function bl_service_worker_version_string(): string
{
	$theme = wp_get_theme(get_template());
	$ver = $theme->exists() ? (string) $theme->get('Version') : '0';

	$base = get_template_directory();
	$sw = bl_service_worker_built_js_path();
 ...
 */
function bl_service_worker_script_url(): string
{
	return add_query_arg([
		'baselayer_sw' => '1',
		'v' => bl_service_worker_version_string(),
	], home_url('/'));
}
 ...
 */
function bl_service_worker_offline_language(): string
{
 ...
 */
function bl_service_worker_offline_html_files(): array
{
...
/**
 * Serve built service worker (src/js/service-worker/index.js) when ?baselayer_sw=1.
 */
add_action('init', function (): void {
	if (!isset($_GET['baselayer_sw']) || (string) wp_unslash($_GET['baselayer_sw']) !== '1') {
		return;
	}
	...
	$path = bl_service_worker_built_js_path();
	if (!is_readable($path)) {
		status_header(404);
	...
	header('Service-Worker-Allowed: ' . bl_service_worker_scope());

	$html = bl_service_worker_offline_html_files();
	$lang = bl_service_worker_offline_language();
	$json_flags = JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT;
	if (defined('JSON_INVALID_UTF8_SUBSTITUTE')) {
		$json_flags |= JSON_INVALID_UTF8_SUBSTITUTE;
	}
	echo 'self.__BL_OFFLINE_HTML__=' . wp_json_encode($html, $json_flags) . ";\n";
	echo 'self.__BL_OFFLINE_LANG__=' . wp_json_encode($lang, $json_flags) . ";\n";
...
add_action('wp_enqueue_scripts', function (): void {
	...
	if (!wp_script_is('baselayer-scripts', 'enqueued')) {
		return;
	}

	wp_localize_script('baselayer-scripts', 'baselayerServiceWorker', [
		'url' => bl_service_worker_script_url(),
		'scope' => bl_service_worker_scope(),
	]);
}, 20);
