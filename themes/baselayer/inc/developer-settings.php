...
function bl_developer_tab_position(string $tab): int
{
	$tabs = array_keys(bl_developer_settings_available_tabs());
	$i = array_search($tab, $tabs, true);
	return $i === false ? 99 : (1 + $i);
}

/** All developer page slugs. */
function bl_developer_settings_page_slugs(): array
{
	$out = [];
	foreach (array_keys(bl_developer_settings_available_tabs()) as $tab) {
		$out[] = bl_developer_settings_page_slug($tab);
	}
	return $out;
}
 ...
 */
function bl_developer_settings_load_hook(string $page_slug): string
{
 ...
 */
function bl_developer_settings_admin_title(string $slug): string
{
...
function bl_developer_settings_screen_heading(): void
{
	$page = isset($_GET['page']) ? sanitize_key((string) $_GET['page']) : '';
	echo '<h1>' . esc_html(bl_developer_settings_admin_title($page)) . '</h1>';
}
...
foreach (bl_developer_settings_page_slugs() as $slug) {
	add_action(bl_developer_settings_load_hook($slug), function () use ($slug) {
		global $title;
		$title = bl_developer_settings_admin_title($slug);
 ...
 * Expects bl_developer_settings_current_tab_from_page() and bl_developer_settings_available_tabs().
 */
function bl_developer_settings_render_nav(): void
{
	$current = bl_developer_settings_current_tab_from_page();
	$tabs = bl_developer_settings_available_tabs();
	...
	if (
		function_exists('bl_config_redis_enabled')
		&& bl_config_redis_enabled()
		&& (
	...
	<nav class="nav-tab-wrapper wp-clearfix" aria-label="Secondary menu">
		...
		foreach ($tabs as $slug => $def) {
			...
			$url = $is_external
				...
				: add_query_arg('page', bl_developer_settings_page_slug($slug), $base_url);
...
$bl_developer_settings_dir = __DIR__ . '/developer-settings/';
require_once $bl_developer_settings_dir . 'developer.php';
...
require_once $bl_developer_settings_dir . 'performance.php';
if (function_exists('bl_theme_feature_enabled') && bl_theme_feature_enabled('languages')) {
	require_once $bl_developer_settings_dir . 'languages.php';
}
...
add_action('admin_menu', function () {
	...
	$hide = array_diff(bl_developer_settings_page_slugs(), [bl_developer_settings_page_slug('developer')]);
...
add_action('admin_menu', function (): void {
	...
	if (!function_exists('bl_config_redis_enabled') || !bl_config_redis_enabled()) {
		return;
	}
	if (
		!class_exists('\RedisCache\Plugin')
...
add_action('load-settings_page_redis-cache', function (): void {
	...
	if (!is_string($title) || $title === '') {
		$title = function_exists('bl_admin_settings_submenu_title')
			? bl_admin_settings_submenu_title(__('Developer', 'baselayer'), __('Redis', 'baselayer'))
			: __('Developer › Redis', 'baselayer');
	}
	if (!current_user_can('manage_options')) {
		return;
	}
...
add_action('admin_notices', function (): void {
	...
	echo '<div class="wrap">';
	echo '<h1>' . esc_html(__('Developer settings', 'baselayer')) . '</h1>';
...
add_filter('submenu_file', function ($submenu_file, $parent_file) {
	...
	if (in_array($page, bl_developer_settings_page_slugs(), true)) {
		return bl_developer_settings_page_slug('developer');
	}
	if (
		$page === 'redis-cache'
...
add_action('admin_bar_menu', function (\WP_Admin_Bar $wp_admin_bar): void {
	...
	foreach (array_keys($nodes) as $id) {
		...
		if (strpos($id_str, 'redis') !== false) {
			if (function_exists('bl_admin_bar_remove_children_of')) {
				bl_admin_bar_remove_children_of($wp_admin_bar, $id_str);
			}
 ...
 */
function bl_clean_revisions(int $keep): int
{
	global $wpdb;
	$deleted = 0;
