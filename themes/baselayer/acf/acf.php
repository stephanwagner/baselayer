 ...
 */
function bl_load_acf_blocks_file(string $path): array
{
	if (!is_readable($path)) {
		return [];
	}
 ...
 * @return list<array<string, mixed>>
 */
function bl_get_acf_blocks(): array
{
	static $blocks = null;
	if ($blocks !== null) {
		return $blocks;
...
$configBlocks = bl_get_acf_blocks();

// Register ACF blocks
function bl_acf_init_core()
{
	global $configBlocks;
	if (!empty($configBlocks) && function_exists('acf_register_block') && function_exists('acf_add_local_field_group')) {
		foreach ($configBlocks as $acfBlock) {
			$registration = [
				...
				'render_callback' => 'bl_acf_block_render_callback',
				'category' => 'design',
				'icon' => $acfBlock['icon'],
			...
			$example = bl_acf_block_inserter_example($acfBlock);
			if ($example !== null) {
...
add_action('acf/init', 'bl_acf_init_core');
 ...
 */
function bl_acf_block_inserter_example(array $acfBlock): ?array
{
	$name = isset($acfBlock['name']) && is_string($acfBlock['name']) ? $acfBlock['name'] : '';

	$preview_data = $name !== '' ? bl_acf_block_preview_data($name) : null;
	if ($preview_data !== null) {
		return bl_acf_block_normalize_example($preview_data, $name);
	}

	$preview_uri = bl_acf_block_preview_image_uri($acfBlock);
	if ($preview_uri === null) {
		return null;
	}
 ...
 * @return array<string, mixed>|null
 */
function bl_acf_block_preview_data(string $block_name): ?array
{
	if ($block_name === '') {
		return null;
	}
 ...
 * @param array<string, mixed> $acfBlock
 */
function bl_acf_block_preview_image_uri(array $acfBlock): ?string
{
	$name = isset($acfBlock['name']) && is_string($acfBlock['name']) ? $acfBlock['name'] : '';
	if ($name === '') {
		return null;
 ...
 */
function bl_acf_block_normalize_example(array $example, string $block_name): array
{
	if (!isset($example['attributes'])) {
		$data = $block_name !== ''
			? bl_acf_block_example_data_expand($block_name, $example)
			: $example;
	...
	if (
		...
		$example['attributes']['data'] = bl_acf_block_example_data_expand(
			$block_name,
 ...
 * @return array<string, mixed>
 */
function bl_acf_block_example_data_expand(string $block_name, array $data): array
{
	$field_map = bl_acf_block_field_map($block_name);
	if ($field_map === []) {
		return $data;
	}
	...
	foreach ($data as $name => $value) {
		...
		if (
			...
			&& ($field['type'] ?? '') === 'repeater'
			&& is_array($value)
			&& bl_acf_is_list_array($value)
		) {
			$expanded = array_merge($expanded, bl_acf_expand_repeater_example($name, $field, $value));
			continue;
		}
 ...
 */
function bl_acf_block_field_map(string $block_name): array
{
	static $cache = [];
 ...
 */
function bl_acf_is_list_array(array $array): bool
{
 ...
 * @return array<string, mixed>
 */
function bl_acf_expand_repeater_example(string $name, array $field, array $rows): array
{
	$out = [
		$name => count($rows),
	];
...
function bl_acf_block_render_callback($block, $content = '', $is_preview = false, $post_id = 0, $wp_block = null, $context = false)
{
 ...
 * @return array<string, mixed>
 */
function bl_acf_block_data(array $block): array
{
 ...
 * @return mixed|null
 */
function bl_acf_block_raw_field_value(array $block, string $field_name)
{
	if ($field_name === '') {
		return null;
	}

	$data = bl_acf_block_data($block);
	if ($data === []) {
		return null;
	}
 ...
 */
function bl_acf_block_field(array $block, string $field_name)
{
	if ($field_name === '') {
		return null;
	}

	$raw = bl_acf_block_raw_field_value($block, $field_name);
	if ($raw !== null && $raw !== false && $raw !== '') {
		return $raw;
	}
 ...
 * @param array<string, mixed> $block
 */
function bl_acf_block_icon_slug(array $block): string
{
	if (empty($block['iconSlug'])) {
		return '';
	}

	return bl_sanitize_icon_slug($block['iconSlug']);
}
 ...
 */
function bl_acf_block_icon_markup(array $block, array $attrs = []): string
{
	$slug = bl_acf_block_icon_slug($block);
	if ($slug === '') {
		return '';
	}
 ...
 */
function bl_acf_block_field_choice_value($value): string
{
	if (is_array($value)) {
...
function bl_acf_toolbars($toolbars)
{
...
add_filter('acf/fields/wysiwyg/toolbars', 'bl_acf_toolbars');

/**
 * ACF options page (Theme-Config).
 */
function bl_acf_add_options_page(): void
{
 ...
 */
function bl_acf_inner_blocks_toolbar_config(): array
{
	global $configBlocks;
	...
	foreach ($configBlocks as $acfBlock) {
		...
		$label = isset($toolbar['label']) ? (string) $toolbar['label'] : __('Add', 'baselayer');
